...
// Tipe untuk data form yang dikelola manual dengan useState
...
const AlarmDialog = ({
  ...
  // Mengelola state form secara manual menggunakan useState
  ...
  // Update state 'selectedDevice' ketika deviceUniqId di form berubah
  useEffect(() => {
    const device =
      devices.find((d) => d.uniqId === formData.deviceUniqId) || null;
    setSelectedDevice(device);
    // Reset 'key' jika device berubah
    if (
      device &&
      formData.key &&
      !Object.keys(device.lastPayload || {}).includes(formData.key)
    ) {
  ...
  // Fungsi generic untuk update state form
  ...
  }, [selectedDevice, formData.keyType]);
