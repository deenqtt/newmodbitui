...
const AlarmDialog = ({
  ...
  const { subscribe, unsubscribe } = useMqtt(); // <-- Gunakan hook MQTT
  ...
  useEffect(() => {
    async function fetchDevices() {
      console.log("[Debug] 1. Fetching devices...");
      try {
        const res = await fetch("/api/devices/for-selection");
        if (!res.ok) throw new Error("Failed to fetch devices");
        const data: DeviceExternal[] = await res.json();
        console.log("[Debug] 2. Devices fetched successfully:", data);
        setDevices(data);
      } catch (error) {
        console.error("[Debug] Error fetching devices:", error);
      }
    }
    if (isOpen) fetchDevices();
  }, [isOpen]);

  // Callback untuk menangani pesan MQTT yang masuk
  const handleMessage = useCallback((topic: string, payloadStr: string) => {
    console.log(`[Debug] 4. Message received on topic '${topic}':`, payloadStr);
    try {
      const outerPayload = JSON.parse(payloadStr);

      if (typeof outerPayload.value === "string") {
        console.log(
          "[Debug] 5a. 'value' field is a string, parsing inner JSON."
        );
        const innerPayload = JSON.parse(outerPayload.value);
        setLivePayload(innerPayload);
        console.log(
          "[Debug] 6a. Inner payload set as live payload:",
          innerPayload
        );
      } else {
        console.log(
          "[Debug] 5b. 'value' field not a string, using outer payload."
        );
        setLivePayload(outerPayload.value || outerPayload); // Ambil 'value' jika ada, jika tidak, ambil seluruh payload
        console.log(
          "[Debug] 6b. Payload set as live payload:",
          outerPayload.value || outerPayload
        );
      }
    } catch (e) {
      console.error("[Debug] Failed to parse MQTT payload:", e);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    if (subscribedTopic && subscribedTopic !== newTopic) {
      console.log(
        `[Debug] 3a. Unsubscribing from old topic: ${subscribedTopic}`
      );
      unsubscribe(subscribedTopic, handleMessage);
    }
    if (newTopic && newTopic !== subscribedTopic) {
      console.log(`[Debug] 3b. Subscribing to new topic: ${newTopic}`);
  ...
  useEffect(() => {
    return () => {
      if (subscribedTopic) {
        console.log(
          `[Debug] Dialog closing. Unsubscribing from: ${subscribedTopic}`
        );
        unsubscribe(subscribedTopic, handleMessage);
      }
    };
  ...
  const payloadKeys = useMemo(() => {
    console.log(
      "[Debug] 7. Recalculating payload keys. Live payload is:",
      livePayload
    );
    ...
    switch (formData.keyType) {
      case "THRESHOLD":
        keys = Object.keys(livePayload).filter((k) => isNumber(livePayload[k]));
        break;
      case "DIRECT":
        keys = Object.keys(livePayload).filter((k) =>
      ...
      case "BIT_VALUE":
        ...
        break;
      default:
        keys = Object.keys(livePayload);
    }
    console.log(
      `[Debug] 8. Filtered keys for type '${formData.keyType}':`,
      keys
    );
    return keys;
