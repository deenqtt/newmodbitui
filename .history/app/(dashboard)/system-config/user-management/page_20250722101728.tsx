...
function UserManagementContent() {
  ...
  const fetchUsers = useCallback(async () => {
    console.log("[UserManagement] Memulai fetchUsers...");
    setIsLoading(true);
    try {
      // Browser akan otomatis mengirim cookie, jadi tidak perlu header Authorization
      const response = await fetch("/api/users");
      console.log(
        "[UserManagement] Response status dari /api/users:",
        response.status
      );
      ...
      console.log("[UserManagement] Data pengguna berhasil diambil:", data);
      setUsers(data);
    } catch (error: any) {
      console.error(
      ...
      console.log("[UserManagement] Proses fetchUsers selesai.");
      setIsLoading(false);
    }
  }, [logout]);
  ...
  useEffect(() => {
    console.log(
      `[UserManagement] Mengecek kondisi: isAuthLoading=${isAuthLoading}, isAuthenticated=${isAuthenticated}, user role=${user?.role}`
    );
    // Hanya jalankan setelah status autentikasi selesai dicek
    if (!isAuthLoading) {
      if (isAuthenticated && user?.role === Role.ADMIN) {
        console.log(
          "[UserManagement] Kondisi terpenuhi, memanggil fetchUsers."
        );
        fetchUsers();
      } else {
        // Jika tidak terotentikasi atau bukan admin, hentikan loading
        console.log(
          "[UserManagement] Kondisi tidak terpenuhi (bukan admin atau belum login), menghentikan loading."
        );
        setIsLoading(false);
      }
    }
