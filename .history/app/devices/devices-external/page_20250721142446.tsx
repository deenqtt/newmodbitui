...
function DevicesExternalContent() {
  ...
  // Gunakan context yang sudah disediakan oleh parent
  const { isReady, connectionStatus, subscribe, unsubscribe } = useMqtt();

  // Log status koneksi setiap kali berubah
  useEffect(() => {
    console.log("[UI] MQTT Connection status updated:", connectionStatus);
  }, [connectionStatus]);
  ...
  const fetchDevices = async () => {
    ...
    try {
      ...
      // LOG HASIL FETCH
      console.log("[API] Devices fetched successfully:", data);
      const fetchedTopics = data.map((d) => d.topic);
      console.log("[API] Topics to be subscribed:", fetchedTopics);
  ...
  useEffect(() => {
    console.log(
      `[Sub Effect] Checking conditions: isReady=${isReady}, topics.length=${topics.length}`
    );

    if (!isReady || topics.length === 0) {
      if (!isReady) console.log("[Sub Effect] Aborting: MQTT not ready.");
      if (topics.length === 0)
        console.log("[Sub Effect] Aborting: No topics to subscribe to yet.");
      return;
    }
    ...
    console.log("[Sub Effect] Proceeding to subscribe to topics:", topics);
    topics.forEach((topic) => subscribe(topic, handleMessage));

    return () => {
      console.log(
        "[Sub Effect] Cleaning up. Unsubscribing from topics:",
        topics
      );
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (response.ok) {
      ...
      fetchDevices(); // Fetch ulang data setelah berhasil
    } else {
