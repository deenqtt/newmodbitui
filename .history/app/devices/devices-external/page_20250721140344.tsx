...
// Define the data type for a device
interface Device {
  id: string;
  uniqId: string;
  name: string;
...
export default function DevicesExternalPage() {
  ...
  // Memoize topics for efficiency
  const topics = useMemo(() => devices.map((d) => d.topic), [devices]);
  const { dbStatus } = useConnectivity(topics);
  const { connectionStatus: mqttStatus, subscribe, unsubscribe } = useMqtt();
  // 2. State untuk payloads dan status DB dibuat terpisah di sini
  const [payloads, setPayloads] = useState<Record<string, string>>({});

  const router = useRouter();
  const importFileRef = useRef<HTMLInputElement>(null);

  // State for various dialogs/modals
