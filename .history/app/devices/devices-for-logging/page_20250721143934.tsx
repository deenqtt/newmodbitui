...
function DevicesForLoggingContent() {
    ...
    const handleMessage = (messageTopic: string, payloadString: string) => {
      ...
      if (messageTopic === topic) {
        ...
        try {
          ...
          console.log(`[Logging Page] Payload updated for ${topic}`, parsedPayload);
        } catch (e) {
          console.error(`[Logging Page] Could not parse payload for ${topic}:`, payloadString, e);
  ...
  const availableKeys = useMemo(() => {
    ...
    try {
      ...
      console.error("[availableKeys] Failed to parse modalPayload.value:", modalPayload.value, e);
                  ...
                  {isLoading ? (
                    ...
                    loggingConfigs.map((config) => (
                      <TableRow key={config.id}>
                        <TableCell className="font-medium">{config.device.name}</TableCell>
