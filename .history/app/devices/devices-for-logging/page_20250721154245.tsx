...
function DevicesForLoggingContent() {
  ...
  const handleDownload = () => {
    if (loggingConfigs.length === 0) {
      Swal.fire("No Data", "There is no configuration to download.", "info");
      return;
    }
    // Siapkan data bersih untuk di-download
    ...
    reader.onload = async (e) => {
      try {
        ...
        Swal.fire({
          ...
          html: `Created: <b>${result.created}</b><br>Skipped: <b>${result.skipped}</b>`,
        });
        fetchInitialData();
      } catch (err: any) {
