...
import { 
  Eye, EyeOff, Move3D, RotateCcw, Info, 
  DoorOpen, DoorClosed, Opacity, Loader2, AlertTriangle
} from "lucide-react";
...
export const RackServer3dWidget = ({ config }: Props) => {
  ...
  const createDevice = async (
    ...
    const materials: THREE.Material[] = [
      ...
      frontTexture 
        ? new THREE.MeshBasicMaterial({ map: frontTexture, side: THREE.DoubleSide }) // Depan
        : new THREE.MeshBasicMaterial({ color: type === 'ac' ? 0x000000 : (type === 'ups' ? 0x333333 : 0x666666), side: THREE.DoubleSide }), // Default color if no image
    ...
    const positionY = -2.15/2 + (positionU * UHeight)/100 + deviceHeight/2;
  ...
  const createScene = async () => {
    ...
    const { panel: leftPanel, material: leftMaterial } = createPanel(
      wallThickness, rackHeight, rackDepth,
      0xcccccc, [-rackWidth/2 + wallThickness/2, 0, 0], 1
    );
    ...
    const { panel: topPanel, material: topMaterial } = createPanel(
      rackWidth, wallThickness, rackDepth,
      0xcccccc, [0, rackHeight/2 - wallThickness/2, 0], 1
    );
    ...
    const { panel: backPanel, material: backMaterial } = createPanel(
      rackWidth, rackHeight, wallThickness,
      0xcccccc, [0, 0, -rackDepth/2 + wallThickness/2], 1
    );
    backPanelRef.current = backPanel;

    // Edges
    const leftEdges = createEdges(wallThickness, rackHeight, rackDepth, [-rackWidth/2 + wallThickness/2, 0, 0]);
    const rightEdges = createEdges(wallThickness, rackHeight, rackDepth, [rackWidth/2 - wallThickness/2, 0, 0]);
    const topEdges = createEdges(rackWidth, wallThickness, rackDepth, [0, rackHeight/2 - wallThickness/2, 0]);
    const bottomEdges = createEdges(rackWidth, wallThickness, rackDepth, [0, -rackHeight/2 + wallThickness/2, 0]);
    const backEdges = createEdges(rackWidth, rackHeight, wallThickness, [0, 0, -rackDepth/2 + wallThickness/2]);
    ...
    if (config.showUPS) {
        devicePromises.push(
            createDevice('ups', config.upsPosition, config.upsHeight, config.upsImageFile, scene).then(() => {})
        );
    }
    ...
    if (config.showServers) {
        config.serverPositions.forEach((server, index) => {
            devicePromises.push(
                createDevice('server', server.position, server.height, server.imageFile, scene).then(() => {})
            );
        });
    }
  ...
  const toggleTransparency = () => {
    ...
    [leftPanelRef.current, rightPanelRef.current, topPanelRef.current, bottomPanelRef.current, backPanelRef.current].forEach(panel => {
          ...
          <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-10">
            ...
            <div className="flex gap-1 bg-white/80 backdrop-blur-sm rounded-lg p-1 shadow-md">
              <Button
                ...
                className="h-8 px-2 text-xs"
              >
                {isDoorOpen ? <DoorOpen className="h-4 w-4 mr-1" /> : <DoorClosed className="h-4 w-4 mr-1" />}
