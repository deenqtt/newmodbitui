    <div v-if="!isZoomedIn">
      <div class="content-style-primary d-flex flex-column gap-1">
        <button class="btn btn-sm btn-primary me-1" @click="toggleLeftCover">
          Front Cover
        </button>
        <button class="btn btn-sm btn-success me-1" @click="restoreAllPanels">
          Restore Cover
        </button>
        <button
        ...
        <button class="btn btn-sm btn-secondary me-1" @click="setView('front')">
          Front View
        </button>
        <button class="btn btn-sm btn-secondary me-1" @click="setView('back')">
          Back View
        </button>
        <button class="btn btn-sm btn-secondary me-1" @click="setView('top')">
          Top View
        </button>
        <button class="btn btn-sm btn-secondary me-1" @click="setView('side')">
          Side View
        </button>
        <button class="btn btn-sm btn-danger me-1" @click="removeAllPanels">
          Remove Cover
        </button>
      </div>
    </div>
...
const mqttBrokerAddress =
  import.meta.env.VITE_ENV === "dev"
    ? import.meta.env.VITE_MQTT_BROKER_ADDRESS
...
const connectClient = async () => {
  try {
    const mappedRacks = await fetchRackData(); // ✅ Ambil data yang sudah dimapping
    if (!mappedRacks.length) {
      console.error("Rack data tidak ditemukan atau invalid.");
      return;
    }

    // 🔥 **Loop untuk setiap rack yang punya topic**
    mappedRacks.forEach((rack) => {
      if (rack.frontTopic) {
        connectClientForRack(rack.frontTopic, rack.rackNumber - 1, "front");
      }
      if (rack.backTopic) {
        connectClientForRack(rack.backTopic, rack.rackNumber - 1, "back");
      }
    });

    // 🔥 **Subscribe ke topicPower untuk cek Power Key**
    if (mappedRacks.some((rack) => rack.powerTopic)) {
      subscribeToPowerTopic(mappedRacks);
    }
  } catch (error) {
    console.error("Failed to connect to MQTT or fetch rack data:", error);
  }
};
...
const fetchRackData = async () => {
  try {
    if (!selectedDeviceId) {
      console.error("No selectedDeviceId found in store.");
      return [];
    }

    // **1. Ambil Container Data (Custom Name)**
    const containerResponse = await axios.get(
      `${appState.value.baseUrl}/api/containers/${selectedDeviceId}`,
      {
        headers: {
    ...
    if (!containerData || !containerData.customName) {
      console.error("Invalid container data:", containerData);
      return [];
    }

    const containerName = containerData.customName;

    // **2. Ambil Rack berdasarkan Custom Name**
    const rackResponse = await axios.get(
      `${appState.value.baseUrl}/racks/${containerName}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("authToken")}`,
        },
    ...
    if (!Array.isArray(rackData) || rackData.length === 0) {
      console.error("No racks found for container:", containerName);
      return [];
    }

    // **3. Ambil Data TopicsTemp dari Container API**
    if (!containerData.topicsTemp || containerData.topicsTemp.length < 2) {
      console.error("Invalid topicsTemp format:", containerData);
      return [];
    }

    const totalRacks = containerData.topicsTemp[0].length;

    // **4. Mapping Data ke `mappedRacks`**
    mappedRacks = []; // **Pastikan `mappedRacks` di-reset sebelum mengisi ulang**
    for (let i = 0; i < totalRacks; i++) {
      const matchingRack = rackData.find((r) => r.name === `rack-${i + 1}`);

      mappedRacks.push({
        id: matchingRack ? matchingRack.id : null, // **Tambahkan ID jika ditemukan di rackData**
        rackNumber: i + 1,
        name: `rack-${i + 1}`, // Nama rack
        frontTopic: containerData.topicsTemp[0][i] || null,
        backTopic: containerData.topicsTemp[1][i] || null,
        powerTopic: containerData.topicPower || null, // Ambil langsung dari topicPower
      });
    }
...
const renderScene = async () => {
  ...
  const joystick = nipplejs.create({
    ...
    mode: "dynamic",
    position: { left: "10%", top: "10%" }, // Pojok kiri atas
    color: "transparent",
    size: Math.min(card.value.clientWidth, card.value.clientHeight) * 0.2, // Ukuran joystick tergantung card
  });
  ...
  joystick.on("move", (evt, data) => {
    ...
    movement.x = x;
    movement.y = y;
    activateJoystick(); // Aktifkan joystick saat mouse ditekan
    controls.update(); // Perbarui OrbitControls
  });

  joystick.on("end", () => {
    movement.x = 0;
    movement.y = 0;
    joystickActive = false; // Nonaktifkan joystick saat dilepas
    controls.update(); // Perbarui OrbitControls
  });
...
const animateCamera = (targetPosition, callback = null) => {
  ...
  const animate = () => {
    ...
    if (t < 1) {
      ...
      // Setelah animasi selesai, perbarui `rotation` ke posisi akhir kamera
      const radius = camera.position.length(); // Jarak kamera dari pusat
      rotation.azimuthAngle = Math.atan2(camera.position.x, camera.position.z);
      rotation.polarAngle = Math.acos(camera.position.y / radius);

      // ✅ **Cek apakah callback adalah fungsi sebelum memanggilnya**
...
const onRackClick = (event) => {
  ...
  if (intersects.length > 0) {
    ...
    if (rackInfo) {
      ...
      // selectedRack.value = clickedRack;

      // originalCameraPosition.copy(camera.position);
      // originalMaterials.set(clickedRack, clickedRack.material.color.getHex());

      // clickedRack.material.color.set(0xff0000);

      // hideAllObjectsExceptSelectedRack(clickedRack);

      // createDetailedRack(clickedRack);

      // // **Posisi Kamera Setelah Zoom-in (Agak Jauh)**
...
const setView = (view) => {
  ...
  switch (view) {
    case "isometric":
      toggleLeftCover();
      targetPosition = new THREE.Vector3(-5, 2, 5); // Isometric View

      break;
    case "front":
      targetPosition = new THREE.Vector3(0, 5, 7.5); // Front View
      break;
    case "top":
      targetPosition = new THREE.Vector3(0, 10, 0); // Top View
      break;
    case "back":
      targetPosition = new THREE.Vector3(0, 5, -7.5); // Back View
      break;
    case "side":
      targetPosition = new THREE.Vector3(10, 2, 0); // Side View
      break;
    default:
...
onMounted(() => {
  ...
  if (card.value) {
    ...
    console.warn(
      "⚠️ card.value belum terisi, tidak bisa menambahkan event listener."
    );
  }
});
</script>