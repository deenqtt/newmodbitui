...
export function ModbusTable() {
  ...
  const deviceTypes = useMemo(() => {
    ...
    // LOG 1: Check if this hook is running and what data it sees
    console.log(`[useMemo] DeviceTypes: Protocol is "${protocol}". Catalog for this protocol exists:`, !!deviceCatalog[protocol]);
    ...
    // LOG 2: See the final list of types found
    console.log("[useMemo] DeviceTypes: Found valid types:", types);
    return types;
  ...
  const manufacturers = useMemo(() => {
    ...
    const uniqueManufacturers = [...new Set(allManufacturers)].filter(Boolean);
    // LOG 3: See the list of manufacturers
    console.log(`[useMemo] Manufacturers: For type "${type}", found:`, uniqueManufacturers);
  ...
  const partNumbers = useMemo(() => {
    ...
    const uniquePartNumbers = [...new Set(allPartNumbers)].filter(Boolean);
     // LOG 4: See the list of part numbers
    console.log(`[useMemo] PartNumbers: For type "${type}", found:`, uniquePartNumbers);
  ...
  const onMessageArrived = (message: Paho.Message) => {
    ...
    if (payload.result === "success" || payload.result === "error") {
      ...
      // LOG 5: See exactly what payload arrives on this topic
      console.log("[onMessageArrived] Received on response_device_modbus:", payload);

      const payloadKeys = Object.keys(payload);
      const isMainDeviceList = payloadKeys.some(key => key.includes("Modbus RTU") || key.includes("SNMP"));

      if (isMainDeviceList) {
        console.log("[onMessageArrived] Identified as MAIN device list for the table.");
        setDevices(payload);
      }
      else if (payloadKeys.length > 0) {
        const currentProtocolInForm = newDevice.protocol_setting.protocol;
        if (currentProtocolInForm) {
          // LOG 6: Confirm that we are about to set the catalog state
          console.log(`[onMessageArrived] Identified as CATALOG data. Associating with protocol: "${currentProtocolInForm}".`);
          setDeviceCatalog(prev => ({
            ...prev,
            [currentProtocolInForm]: payload
          }));
        } else {
            console.warn("[onMessageArrived] Received catalog-like data, but no protocol is selected in the form state.");
        }
      }
  ...
  }, [client, connectionStatus, newDevice.protocol_setting.protocol]); // Added dependency to re-run if protocol changes
  ...
  const renderModbusFields = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="md:col-span-2">
        ...
        <Input id="deviceName" value={newDevice.profile.name} onChange={(e) => setNewDevice((p: any) => ({ ...p, profile: { ...p.profile, name: e.target.value } }))} required />
      </div>
      {renderSharedDynamicFields()}
      <div>
        <Label htmlFor="modbusAddress">Address</Label>
        <Input id="modbusAddress" type="number" value={newDevice.protocol_setting.address || ""} onChange={(e) => setNewDevice((p: any) => ({ ...p, protocol_setting: { ...p.protocol_setting, address: Number(e.target.value) } }))} required />
      </div>
      <div>
        <Label htmlFor="modbusPort">Port</Label>
        <Select value={newDevice.protocol_setting.port} onValueChange={(v) => setNewDevice((p: any) => ({ ...p, protocol_setting: { ...p.protocol_setting, port: v } }))} required>
          <SelectTrigger id="modbusPort"><SelectValue placeholder="Select Port" /></SelectTrigger>
          <SelectContent>
      ...
      <div className="md:col-span-2">
        ...
        <Input id="topic" value={newDevice.profile.topic} onChange={(e) => setNewDevice((p: any) => ({ ...p, profile: { ...p.profile, topic: e.target.value } }))} required />
      </div>
    </div>
  );
  ...
  const renderSnmpFields = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="md:col-span-2">
            ...
            <Input id="deviceNameSnmp" value={newDevice.profile.name} onChange={(e) => setNewDevice((p: any) => ({ ...p, profile: { ...p.profile, name: e.target.value } }))} required />
        </div>
        {/* FIX: Adding the shared dynamic fields to the SNMP form as well */}
        {renderSharedDynamicFields()}
        <div>
            <Label htmlFor="snmpIp">SNMP IP Address</Label>
            <Input id="snmpIp" value={newDevice.protocol_setting.ip_address} onChange={(e) => setNewDevice((p: any) => ({ ...p, protocol_setting: { ...p.protocol_setting, ip_address: e.target.value } }))} required />
        </div>
        <div>
            <Label htmlFor="snmpPort">SNMP Port</Label>
            <Input id="snmpPort" type="number" value={newDevice.protocol_setting.port} onChange={(e) => setNewDevice((p: any) => ({ ...p, protocol_setting: { ...p.protocol_setting, port: Number(e.target.value) } }))} required />
        </div>
        <div className="md:col-span-2">
            <Label htmlFor="topicSnmp">MQTT Topic</Label>
            <Input id="topicSnmp" value={newDevice.profile.topic} onChange={(e) => setNewDevice((p: any) => ({ ...p, profile: { ...p.profile, topic: e.target.value } }))} required />
        </div>
    </div>
  );
    ...
    <div className="p-4 border rounded-lg mt-4 bg-card text-card-foreground shadow-sm">
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {pagedDevices.length > 0 ? (
              pagedDevices.map((device, index) => (
                <TableRow key={device.profile?.name || `device-${index}`}>
                  ...
                  <TableCell className="font-medium">{device.profile?.name}</TableCell>
                  <TableCell>{device.profile?.part_number}</TableCell>
                  <TableCell>
                    {device.protocol_setting?.protocol === "SNMP" ? (
                      <span className="text-blue-500 cursor-pointer hover:underline" onClick={() => pingIp(device.protocol_setting?.ip_address)} title={`Ping ${device.protocol_setting?.ip_address}`}>
            ...
            ) : (<TableRow><TableCell colSpan={7} className="text-center h-24">No data found. Click "Get All Data" to fetch devices.</TableCell></TableRow>)}
          </TableBody>
        </Table>
      </div>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>{isUpdateMode ? "Update Device" : "Add New Device"}</DialogTitle></DialogHeader>
          <form onSubmit={handleFormSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto p-4">
            ...
            {newDevice.protocol_setting?.protocol === "Modbus RTU" && renderModbusFields()}
            {newDevice.protocol_setting?.protocol === "SNMP" && renderSnmpFields()}
            <DialogFooter>
              <DialogClose asChild><Button type="button" variant="secondary">Close</Button></DialogClose>
              <Button type="submit">{isUpdateMode ? "Update Device" : "Add Device"}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isSpecialModalOpen} onOpenChange={setIsSpecialModalOpen}>
        <DialogContent className="max-w-3xl">
          <DialogHeader><DialogTitle>Update Special Device</DialogTitle></DialogHeader>
          <form onSubmit={handleFormSubmit} className="space-y-4 max-h-[70vh] overflow-y-auto p-4">
