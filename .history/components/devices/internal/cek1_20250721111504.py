...
MODULAR_I2C_CONFIG_PATH = '../MODULAR_I2C/JSON/Config/installed_devices.json'
I2C_CONFIG_PATH = '../I2C/JSON/Config/installed_devices.json'
...
MQTT_I2C_CONFIG_PATH = '../I2C/JSON/Config/mqtt_config.json'
...
DEFAULT_DEVICES_PATH = '../MODBUS_SNMP/JSON/Config/installed_devices.json'

# Load default devices from file
def load_default_devices(client):
    try:
        with open(DEFAULT_DEVICES_PATH, 'r') as file:
            content = file.read()
            if not content:  # Handle empty file
                return []
            return json.loads(content)
    except FileNotFoundError:
        log_error(client, f"File {DEFAULT_DEVICES_PATH} not found", "major")
        return []
    except json.JSONDecodeError as e:
        log_error(client, f"Error decoding JSON from {DEFAULT_DEVICES_PATH}: {e}", "major")
        return []
...
def handle_modbus_snmp_message(client, userdata, message):
    ...
    try:
        ...
        installed_devices = load_installed_devices(client, DEFAULT_DEVICES_PATH)
        response = {}
...
def handle_i2c_message(client, userdata, message):
    ...
    installed_devices = load_installed_devices(client, MODULAR_I2C_CONFIG_PATH)
    ...
    try:
        ...
        if command.get('command') == 'getDataI2C':
            ...
            save_installed_devices(client, MODULAR_I2C_CONFIG_PATH, installed_devices)
            response = {"status": "success", "message": "Device added successfully"}
...
# Handle incoming MQTT messages for I2C Plain (modbit)
def handle_i2c_plain_message(client, userdata, message):
    print(f"Received I2C PLAIN message: {message.payload.decode('utf-8')} on topic {message.topic}")
    payload = message.payload.decode('utf-8')
    ...
    try:
        ...
        if command.get('command') == 'getDataI2C':
            print("Processing getDataI2C (plain) command")
            response = installed_devices
            ...
            for device in installed_devices:
                if device['profile']['name'] == new_device['profile']['name'] or device['protocol_setting']['address'] == new_device['protocol_setting']['address']:
                    response = {"status": "error", "message": "Device with the same name or address already exists."}
                    client.publish(I2C_PLAIN_RESPONSE_TOPIC, json.dumps(response), qos=1, retain=False)
                    return
            ...
            print("Processing checkI2CAddresses (plain) command")
            try:
                i2c_result = check_i2c_addresses(client)
                response = {"status": "success", "data": i2c_result}
            except Exception as e:
                log_error(client, f"Error checking I2C addresses (plain): {e}", "major")
                response = {"status": "error", "message": str(e)}

        else:
            response = {"status": "error", "message": "Unknown command"}

        client.publish(I2C_PLAIN_RESPONSE_TOPIC, json.dumps(response), qos=0, retain=False)

    except json.JSONDecodeError as e:
        log_error(client, f"Error decoding I2C plain message payload: {e}", "major")
    except Exception as e:
        log_error(client, f"Error processing I2C plain message: {e}", "major")
...
def check_i2c_addresses(client):
    try:
        result = subprocess.run(['sudo', 'i2cdetect', '-y', '1'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def get_local_mac_ip_username(client):
    try:
        ...
        username = "npi"  # Or use getpass.getuser() if you want the current logged-in user
...
def send_device_data(client, device_type, data):
    try:
        ...
        if device_type == "modbus":
            topic = MODBUS_SNMP_DATA_TOPIC
        elif device_type == "i2c":
            topic = I2C_DATA_TOPIC
        elif device_type == "i2c_plain":
            topic = I2C_PLAIN_DATA_TOPIC
        else:
            topic = "unknown/data"  # fallback
    ...
    try:
        ...
        try:
            ...
            if command == "device_modbus":
                ...
                devices = load_installed_devices(client, MODULAR_I2C_CONFIG_PATH)
...
I2C_PLAIN_COMMAND_TOPIC = "command_device_i2c_plain"
I2C_PLAIN_RESPONSE_TOPIC = "response_device_i2c_plain"
I2C_PLAIN_DATA_TOPIC = "data_device_i2c_plain_node"
...
def periodic_publish(client):
    while True:
        try:
            ...
            i2c_devices_modular = load_installed_devices(client, MODULAR_I2C_CONFIG_PATH)
            i2c_devices_plain = load_installed_devices(client, I2C_CONFIG_PATH)

            send_device_data(client, "modbus", modbus_devices)
            send_device_data(client, "i2c", i2c_devices_modular)
            send_device_data(client, "i2c_plain", i2c_devices_plain)  # Publish ke topic baru
        except Exception as e:
...
def on_connect_operations(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.subscribe(I2C_PLAIN_COMMAND_TOPIC)  # subscribe topik baru
...
def setup_mqtt_client_operations():
    ...
    client.message_callback_add(I2C_PLAIN_COMMAND_TOPIC, handle_i2c_plain_message)  # handler baru
