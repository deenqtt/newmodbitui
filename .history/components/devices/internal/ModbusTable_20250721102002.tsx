...
export function ModbusTable() {
  ...
  const manufacturers = useMemo(() => {
    ...
    return [...new Set(allManufacturers)].filter(Boolean); // Return unique, non-empty values
  }, [
  ...
  const partNumbers = useMemo(() => {
    ...
    return [...new Set(allPartNumbers)].filter(Boolean); // Return unique, non-empty values
  }, [
    newDevice.protocol_setting?.protocol,
    newDevice.profile?.device_type,
    deviceCatalog,
  ...
  const onMessageArrived = (message: Paho.Message) => {
    ...
    if (payload.result === "success" || payload.result === "error") {
      ...
      // Heuristic: Check if the payload structure is for the main device table (keys are protocols)
      const isMainDeviceList = payloadKeys.some(
        (key) => key.includes("Modbus RTU") || key.includes("SNMP")
      );

      if (isMainDeviceList) {
        setDevices(payload);
      }
      console.log("Received devices:", payload);
      // Otherwise, assume it's a catalog payload for the dropdowns
      else if (payloadKeys.length > 0) {
        const currentProtocolInForm = newDevice.protocol_setting.protocol;
        // Ensure we have a protocol selected in the form to associate the catalog with
  ...
  const handleProtocolChange = (protocol: string) => {
    // Set the protocol in the form state FIRST, so onMessageArrived has context
  ...
  const renderSnmpFields = () => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      {/* The dropdowns for Device Type, Manufacturer, etc., will now appear here too */}
      <div className="md:col-span-2">
      ...
      <div>
        <Label htmlFor="deviceTypeSnmp">Device Type</Label>
        <Select
          value={newDevice.profile.device_type}
          onValueChange={handleDeviceTypeChange}
        >
          <SelectTrigger id="deviceTypeSnmp">
            <SelectValue placeholder="Select Device Type" />
          </SelectTrigger>
          <SelectContent>
            {deviceTypes.map((type) => (
      ...
      <div>
        <Label htmlFor="manufacturerSnmp">Manufacturer</Label>
        <Select
          value={newDevice.profile.manufacturer}
          onValueChange={(v) =>
            setNewDevice((p: any) => ({
          ...
          disabled={!newDevice.profile.device_type}
        >
          <SelectTrigger id="manufacturerSnmp">
            <SelectValue placeholder="Select Manufacturer" />
          </SelectTrigger>
          <SelectContent>
            {manufacturers.map((m) => (
      ...
      <div>
        <Label htmlFor="partNumberSnmp">Part Number</Label>
        <Select
          value={newDevice.profile.part_number}
          onValueChange={(v) =>
            setNewDevice((p: any) => ({
          ...
          disabled={!newDevice.profile.device_type}
        >
          <SelectTrigger id="partNumberSnmp">
            <SelectValue placeholder="Select Part Number" />
          </SelectTrigger>
          <SelectContent>
            {partNumbers.map((pn) => (
