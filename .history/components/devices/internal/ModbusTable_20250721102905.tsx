...
export function ModbusTable() {
  ...
  // --- STATE MANAGEMENT ---
  const [devices, setDevices] = useState<any[]>([]); // Default to array now
  ...
  const pagedDevices = useMemo(() => {
    ...
    // devices is now always an array
  ...
  const onMessageArrived = (message: Paho.Message) => {
    ...
    if (payload.result === "success" || payload.result === "error") {
      ...
      // Cara baru yang lebih andal untuk membedakan payload
      // Jika payload adalah ARRAY, itu untuk tabel utama.
      if (Array.isArray(payload)) {
        console.log(
          "[onMessageArrived] Diidentifikasi sebagai data DAFTAR DEVICE (karena merupakan Array)."
        );
        setDevices(payload);
      }
      // Jika payload adalah OBJECT (tapi bukan null dan bukan array), itu untuk katalog.
      else if (typeof payload === "object" && payload !== null) {
        const currentProtocolInForm = newDevice.protocol_setting.protocol;
        if (currentProtocolInForm) {
          console.log(
            `[onMessageArrived] Diidentifikasi sebagai data KATALOG. Mengasosiasikan dengan protokol: "${currentProtocolInForm}".`
          );
