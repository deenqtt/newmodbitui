...
export function ModbusTable() {
  ...
  const deviceTypes = useMemo(() => {
    ...
    const types = Object.keys(deviceCatalog[protocol]).filter(
      (type) =>
        Array.isArray(deviceCatalog[protocol][type]) &&
        deviceCatalog[protocol][type].length > 0
    );
    return types;
  ...
  // 1. Bungkus onMessageArrived dengan useCallback
  const onMessageArrived = useCallback(
    (message: Paho.Message) => {
      ...
      if (payload.result === "success" || payload.result === "error") {
        // ... (logika swal tetap sama)
      } else if (message.destinationName === "response_device_modbus") {
        // Logika di dalam ini sudah benar, masalahnya adalah 'closure' nya
        if (Array.isArray(payload)) {
          setDevices(payload);
        } else if (typeof payload === "object" && payload !== null) {
          // Karena `onMessageArrived` sekarang bergantung pada `newDevice`,
          // `newDevice.protocol_setting.protocol` di sini akan selalu yang terbaru.
        ...
        // ... (logika restart tetap sama)
      }
      // 2. Tambahkan `newDevice` sebagai dependensi.
      // Ini berarti `useCallback` akan membuat ulang fungsi ini jika `newDevice` berubah.
    },
    [newDevice]
  );
  ...
  useEffect(() => {
    if (client && connectionStatus === "Connected") {
      ...
      // 3. Daftarkan handler yang sudah di-memoize oleh useCallback
      client.onMessageArrived = onMessageArrived;

      // Panggil getAllData hanya sekali saat koneksi pertama kali terbentuk
      if (devices.length === 0) {
        getAllData();
      }
    }
    // 4. `useEffect` sekarang bergantung pada `onMessageArrived`.
    // Ia akan berjalan lagi untuk mendaftarkan handler baru ke Paho MQTT
    // setiap kali `onMessageArrived` diperbarui oleh `useCallback`.
  }, [client, connectionStatus, onMessageArrived, devices.length]);

  // --- ACTIONS (Semua fungsi di bawah ini tidak berubah) ---
  ...
  // ... (sisa fungsi action lainnya tetap sama)
  ...
  // --- FORM FIELD RENDERERS (Tidak ada perubahan) ---
