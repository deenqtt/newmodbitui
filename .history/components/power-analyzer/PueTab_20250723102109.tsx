...
export function PueTab() {
  ...
  // Mengambil devices for selection, dan mencoba memuat payload dari localStorage
        ...
        data.map((device: DeviceForSelection) => {
          ...
          try {
            const storedPayload = localStorage.getItem(
              `mqttPayload_${device.uniqId}`
            );
            if (storedPayload) {
              lastKnownPayload = JSON.parse(storedPayload);
            }
          } catch (e) {
            console.warn(
              `[PUE Tab] Failed to parse stored payload for ${device.uniqId}:`,
              e
            );
            // Hapus item jika corrupt
            localStorage.removeItem(`mqttPayload_${device.uniqId}`);
          }
          return { ...device, payload: lastKnownPayload }; // Gunakan payload dari localStorage jika ada
        })
      );
    } catch (error) {
  ...
  // handleMqttMessage: sekarang menyimpan payload ke localStorage
  const handleMqttMessage = useCallback(
    (topicName: string, messageString: string) => {
      ...
      setDevicesForSelection((prevDevices) =>
        prevDevices.map((d) => {
          if (d.topic === topicName) {
            ...
            // );
            // NEW: Simpan payload ke localStorage
            try {
              localStorage.setItem(
                `mqttPayload_${d.uniqId}`,
                JSON.stringify(innerValuePayload)
              );
            } catch (e) {
              console.error(
                "[PUE Tab] Failed to save payload to localStorage:",
                e
              );
            }
      ...
      // --- END Update form's filtered keys ---
      ...
      setPueConfigs((prevConfigs) => {
        ...
        const newConfigs = prevConfigs.map((config) => {
          ...
          const newPduList = config.pduList.map((pdu) => {
            const pduDeviceFromState = devicesForSelection.find(
              (d) => d.uniqId === pdu.topicUniqId
            );
            // Mengambil topik dari device state yang sudah di-update
            const pduTopicName = pduDeviceFromState?.topic;
          ...
          const mainPowerDeviceFromState = devicesForSelection.find(
            (d) => d.uniqId === config.mainPower.topicUniqId
          );
          const mainPowerTopicName = mainPowerDeviceFromState?.topic;
    ...
    // devicesForSelection sebagai dependency agar updates ke filteredKeys bekerja
  ...
  const subscribeToRelevantTopics = useCallback(
    async (configs: PueConfig[]) => {
      ...
      await Promise.all(topicFetchPromises);
  ...
  // fetchData: sekarang menginisialisasi value dari devicesForSelection
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // Pastikan devicesForSelection sudah terisi dengan payload terbaru
      // Jika fetchDevicesForSelection belum selesai, ini bisa jadi race condition.
      // Solusi terbaik adalah memastikan ini selesai duluan atau digabungkan.
      // Untuk tujuan ini, kita akan mengasumsikan devicesForSelection sudah up-to-date.
      const currentDevicesForSelection = devicesForSelection;

      const fetchedPueConfigs: PueConfig[] = data
        .filter((item) => item.type === "pue")
        .map((item) => {
          // Inisialisasi Main Power Value dari devicesForSelection yang sudah ada payloadnya
          const mainPowerDevice = currentDevicesForSelection.find(
            (d) => d.uniqId === item.mainPower.topicUniqId
          );
          let currentMainPowerValue: number | null = null;
          if (mainPowerDevice && mainPowerDevice.payload) {
            const val = mainPowerDevice.payload[item.mainPower.key];
            if (val !== undefined && !isNaN(parseFloat(val))) {
              currentMainPowerValue = parseFloat(val);
            }
          }

          // Inisialisasi PDU List Values dari devicesForSelection yang sudah ada payloadnya
          const updatedPduList = (item.pduList || []).map((pdu) => {
            const pduDevice = currentDevicesForSelection.find(
              (d) => d.uniqId === pdu.topicUniqId
            );
            ...
            return {
              ...pdu,
              value: totalPduValue,
              topic: pduDevice, // Agar data device lengkap saat diedit/dilihat
            };
          });
          ...
          return {
            ...
            mainPower: {
              ...item.mainPower,
              value: currentMainPowerValue,
              topic: mainPowerDevice, // Agar data device lengkap saat diedit/dilihat
            },
          };
        });

      fetchedPueConfigs.forEach((config) => {
        // PUE akan dihitung dengan nilai yang sudah diambil dari localStorage / MQTT (jika sudah tiba)
    ...
    devicesForSelection,
  ]); // Tambahkan devicesForSelection ke dependencies
  ...
  const editItem = (id: string) => {
    ...
    if (itemToEdit) {
      ...
      setMainPowerForm({
        ...
        // NEW: Inisialisasi value dengan data dari devicesForSelection (termasuk localStorage)
        value:
          mainPowerDeviceObj?.payload?.[itemToEdit.mainPower.key] !==
            undefined &&
          !isNaN(
            parseFloat(mainPowerDeviceObj.payload[itemToEdit.mainPower.key])
          )
            ? parseFloat(mainPowerDeviceObj.payload[itemToEdit.mainPower.key])
            : null,
      ...
      const newPduListForm = itemToEdit.pduList.map((pdu) => {
        ...
        let totalPduValue: number | null = null;
        if (pduDeviceObj && pduDeviceObj.payload) {
          let sumKeys = 0;
          let hasValidValue = false;
          pdu.keys.forEach((key) => {
            if (key in pduDeviceObj.payload) {
              const val = parseFloat(pduDeviceObj.payload[key]);
              if (!isNaN(val)) {
                sumKeys += val;
                hasValidValue = true;
              }
    ...
    try {
      const payload: Partial<PueConfig> = {
        ...
        pduList: pduListForm.map((pdu, index) => ({
          ...
          name: pdu.name || `PDU-${index + 1}`,
          keys: pdu.keys,
          value: null, // Tetap null saat update ke backend
        })),
        mainPower: {
          topicUniqId: mainPowerForm.topicUniqId,
          key: mainPowerForm.key,
  ...
  // Pastikan fetchDevicesForSelection dijalankan duluan
  useEffect(() => {
    fetchDevicesForSelection();
  }, [fetchDevicesForSelection]);

  useEffect(() => {
    // Jalankan fetchData setelah devicesForSelection terisi
    if (devicesForSelection.length > 0 || pueConfigs.length > 0) {
      // Cek agar tidak looping jika devicesForSelection masih kosong
      fetchData();
    }
  ...
  }, [fetchData, unsubscribe, handleMqttMessage, devicesForSelection]); // Tambahkan devicesForSelection sebagai dependensi
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {item.pue === "N/A" &&
                      (item.mainPower.value === null ||
                        item.pduList.some((p) => p.value === null)) ? (
