...
export function PueTab() {
  ...
  // NEW: Ref to keep track of currently subscribed topics *within the form context*
  ...
  const fetchDevicesForSelection = useCallback(async () => {
    ...
    try {
      ...
      // Initialize payload to an empty object, it will be filled by MQTT messages
  ...
  // Modified handleMqttMessage to also update devicesForSelection's payload
  const handleMqttMessage = useCallback(
    (topicName: string, messageString: string) => {
      ...
      // --- NEW: Update form's filtered keys based on new payload ---
      // This ensures that when a new MQTT message arrives, the dropdown options are updated
      setPduListForm((prev) => {
        let updated = false;
      ...
      // Update pueConfigs based on incoming MQTT data (original logic)
    ...
    [calculatePUE, devicesForSelection] // Add devicesForSelection to dependencies
  );
  ...
  const resetForm = useCallback(() => {
    // NEW: Unsubscribe from all topics tracked by the form when resetting
  ...
  }, [unsubscribe, handleMqttMessage]); // Add unsubscribe, handleMqttMessage to dependencies
  ...
  const handleRemovePdu = (index: number) => {
    // NEW: Unsubscribe from the specific PDU's topic if it was subscribed for the form
    setPduListForm((prev) => {
  ...
  const updatePduTopic = async (index: number, selectedTopicUniqId: string) => {
    ...
    // Get the previously selected topic's name for this PDU
    ...
    // Unsubscribe from the old topic if it exists and was subscribed for the form
    ...
    // Subscribe to the new topic if it exists and is not already subscribed by the main config
    if (
      newTopicName &&
      !topicUniqIdToTopicNameMapRef.current.has(selectedTopicUniqId) && // Check if not subscribed by main config
      !formTopicSubscriptionRef.current.has(newTopicName) // Check if not already subscribed by another form element
    ) {
    ...
    // Update the PDU form state
    setPduListForm((prev) => {
      const newPduList = [...prev];
      const currentPdu = newPduList[index];
      currentPdu.topicUniqId = selectedTopicUniqId;
      currentPdu.topic = newDeviceObj; // Assign the full device object

      // If the new device object already has a payload, use it to filter keys immediately
      ...
      currentPdu.keys = []; // Reset selected keys
  ...
  const updateMainPowerTopic = async (selectedTopicUniqId: string) => {
    ...
    // Get the previously selected topic's name for Main Power
    const prevTopicName = devicesForSelection.find(
    ...
    // Update the Main Power form state
    setMainPowerForm((prev) => {
      ...
      return {
        ...
        topic: newDeviceObj, // Assign the full device object
        filteredKeys: filteredKeys,
        key: "", // Reset selected key
      };
    });
  };
  ...
  const handleAddData = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm(); // This will also unsubscribe from form-related topics
      await fetchData(); // Refetch all configs and manage subscriptions for them
      await fetchDevicesForSelection(); // Ensure devices list is fresh
    } catch (error: any) {
  ...
  const editItem = (id: string) => {
    ...
    if (itemToEdit) {
      ...
      // NEW: Subscribe to mainPower topic for editing, if not already subscribed by main configs
      ...
      const newPduListForm = itemToEdit.pduList.map((pdu) => {
        ...
        // NEW: Subscribe to PDU topic for editing, if not already subscribed by main configs
        if (
          pduDeviceObj?.topic &&
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // NEW: Also unsubscribe from any form-related topics on unmount
      formTopicSubscriptionRef.current.forEach((topicName) => {
        unsubscribe(topicName, handleMqttMessage);
      });
      formTopicSubscriptionRef.current.clear();
  ...
  // NEW: Effect to handle modal closing - important for unsubscribing form-related topics
  useEffect(() => {
    if (!isAddModalOpen && !isEditModalOpen) {
      // If both modals are closed, ensure form topics are cleaned up
      resetForm();
    }
                  ...
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      ...
                      (item.mainPower.value === null ||
                        ...
                        <span>Calculating...</span> // Atau ikon loading
                      ) : (
                        item.pue
                      )}
                      <Button
