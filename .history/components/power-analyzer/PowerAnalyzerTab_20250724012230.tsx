...
export function PowerAnalyzerTab() {
  ...
  const [liveSensorValues, setLiveSensorValues] = useState<
    Record<string, { mainPower: number | null; pduValues: Record<string, number | null> }>
  ...
  const [selectedItemForDetail, setSelectedItemForDetail] = useState<PowerAnalyzerConfig | null>(null); // For Detail Modal
    ...
    (topicName: string, messageString: string) => {
      ...
      setLiveSensorValues((prevLiveValues) => {
        ...
        currentConfigs.forEach((config) => {
          ...
          const currentConfigLiveValues = newLiveValues[config.id] || { mainPower: null, pduValues: {} };
          ...
          config.pduList.forEach((pdu) => {
            ...
            if (device && device.topicName === topicName) {
              ...
              if (currentConfigLiveValues.pduValues[pdu.topicId] !== totalPduValue) {
                currentConfigLiveValues.pduValues[pdu.topicId] = totalPduValue;
                // Update the config's pduList value directly for rendering
                config.pduList.find(p => p.topicId === pdu.topicId)!.value = totalPduValue;
                configsUpdated = true;
          ...
          if (config.mainPower) {
            const device = currentDevices.find(
              (d) => d.topicName === topicName && d.id === parseInt(config.mainPower!.topicId)
            );
            if (device && device.topicName === topicName) {
  ...
  const updatePduValuesInBackend = useCallback(async (puePdu: PowerAnalyzerConfig) => {
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`Error updating PDU values for ID: ${puePdu.id}`, errorData);
      }
    } catch (error) {
  ...
  const fetchDevicesForSelection = useCallback(async () => {
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.message || "Failed to fetch devices for selection.");
      }
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!configsRes.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch power analyzer configurations.");
      }
      ...
      if (!logsRes.ok) {
        ...
        throw new Error(errorData.message || "Failed to fetch power analyzer logs.");
      }
      ...
      const processedConfigs: PowerAnalyzerConfig[] = configsData
        ...
        .map((config) => {
          ...
          const pduListParsed = config.pduListJson ? JSON.parse(config.pduListJson) : [];
          const mainPowerParsed = config.mainPowerJson ? JSON.parse(config.mainPowerJson) : null;

          const currentDevices = devicesForSelectionRef.current; // Menggunakan ref

          // Map PDU list
          const pduListMapped: PduConfig[] = (pduListParsed || []).map((pdu: any) => {
            const device = currentDevices.find(d => d.id === parseInt(pdu.topicId));
            const pduValue = liveSensorValues[config.id]?.pduValues[pdu.topicId] || null; // Use live value if available
            return {
              topicId: pdu.topicId,
              ...
              topic: device,
              topicName: device?.topicName,
              filteredKeys: Object.keys(device?.payload || {}).filter(key => key.toLowerCase().includes("power")),
            };
          });
          ...
          if (mainPowerParsed) {
            const device = currentDevices.find(d => d.id === parseInt(mainPowerParsed.topicId));
            const mainPowerValue = liveSensorValues[config.id]?.mainPower || null; // Use live value if available
            mainPowerMapped = {
  ...
  }, [getAuthHeaders, subscribe, unsubscribe, handleMqttMessage, liveSensorValues]); // liveSensorValues sebagai dependensi karena digunakan untuk inisialisasi
  ...
  const addPduToForm = () => { // Renamed from addSensorToForm
    setPduListForm((prev) => [
      ...prev,
      { topicId: "", name: "", keys: [], filteredKeys: [], value: null },
    ]);
  ...
  const removePduFromForm = (index: number) => { // Renamed from removeSensorFromForm
    setPduListForm((prev) => {
    ...
    async (index: number, selectedTopicId: string) => {
      setPduListForm((prev) => {
        ...
        newList[index] = {
          ...
          filteredKeys: Object.keys(selectedDeviceObj?.payload || {}).filter(key => key.toLowerCase().includes("power")),
  ...
  const handleAddData = async (e: FormEvent) => {
    e.preventDefault();
    if (!customName || !mainPowerForm.topicId || !mainPowerForm.key || pduListForm.some(p => !p.topicId || p.keys.length === 0)) {
    ...
    try {
      ...
      if (!apiTopicResponse.ok) {
        ...
        throw new Error(errorData.message || "Failed to create API topic device.");
      }
      ...
      const payload = {
        ...
        pduListJson: JSON.stringify(pduListForm.map((pdu, index) => ({
          topicId: pdu.topicId,
    ...
    (configId: string) => {
      ...
      if (configToEdit.mainPower) {
        ...
        setMainPowerForm({
          ...
          filteredKeys: Object.keys(mainPowerDevice?.payload || {}).filter(key => key.toLowerCase().includes("power")),
        });
        // Subscribe to main power topic for key filtering
        if (mainPowerDevice?.topicName && !formTopicSubscriptionsRef.current.has(mainPowerDevice.topicName)) {
          subscribe(mainPowerDevice.topicName, handleMqttMessage);
          formTopicSubscriptionsRef.current.set(mainPowerDevice.topicName, mainPowerDevice.topicName);
        }
      } else {
        setMainPowerForm({ topicId: "", key: "", filteredKeys: [], value: null });
      }
      ...
      const preparedPduListForm = configToEdit.pduList.map((pdu) => {
        ...
        if (pduDevice?.topicName && !formTopicSubscriptionsRef.current.has(pduDevice.topicName)) {
          subscribe(pduDevice.topicName, handleMqttMessage);
          formTopicSubscriptionsRef.current.set(pduDevice.topicName, pduDevice.topicName);
        }
        return {
          ...pdu,
          topic: pduDevice,
          filteredKeys: Object.keys(pduDevice?.payload || {}).filter(key => key.toLowerCase().includes("power")),
        };
      });
  ...
  const handleEditData = async (e: FormEvent) => {
    ...
    if (
      !selectedConfigId ||
      !customName ||
      !mainPowerForm.topicId || !mainPowerForm.key ||
    ...
    try {
      ...
      if (configToDelete.apiTopicId) {
        ...
        if (!topicDeleteResponse.ok) {
          ...
          console.error(`Failed to delete associated API topic: ${errorData.message}`);
  ...
  const handleDeleteAllLogs = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(
          errorData.message || "Failed to delete all logs."
        );
      }
    ...
    currentConfigs.forEach(config => {
      ...
      if (liveDataForConfig) {
        ...
        const tempPduList: PduConfig[] = config.pduList.map(pdu => ({
          ...pdu,
          value: liveDataForConfig.pduValues[pdu.topicId] !== undefined ? liveDataForConfig.pduValues[pdu.topicId] : pdu.value
        }));
        const tempMainPower: MainPowerConfig | null = config.mainPower ? {
          ...config.mainPower,
          value: liveDataForConfig.mainPower !== undefined ? liveDataForConfig.mainPower : config.mainPower.value
        } : null;

        // Check if values actually changed before calling backend
        const hasPduValueChanges = config.pduList.some((pdu, idx) => pdu.value !== tempPduList[idx].value);
        const hasMainPowerValueChange = config.mainPower?.value !== tempMainPower?.value;
              ...
              {isLoadingInitialData ? (
                ...
                paginatedConfigs.map((config, index) => (
                  <TableRow key={config.id}>
                    ...
                    <TableCell>
                      {calculatePUE(
                        liveSensorValues[config.id]?.mainPower !== undefined ? liveSensorValues[config.id]?.mainPower : config.mainPower?.value,
                        config.pduList.map(pdu => ({
                          ...pdu,
                          value: liveSensorValues[config.id]?.pduValues[pdu.topicId] !== undefined ? liveSensorValues[config.id]?.pduValues[pdu.topicId] : pdu.value
                        }))
                      )}
                      <Button
                        variant="outline"
            ...
            <div className="grid gap-4 py-4">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  required
                  disabled={
                    isSubmitting || (mainPowerForm.filteredKeys?.length || 0) === 0
                  }
                >
                  <SelectTrigger>
                    <SelectValue
        ...
        <DialogContent className="sm:max-w-[800px]">
          ...
          <form onSubmit={handleEditData}>
            <div className="grid gap-4 py-4">
              ...
              <h6 className="font-semibold mt-3">Main Power</h6>
              <div className="space-y-2">
                <Label htmlFor="mainPowerTopicSelectionEdit">Select Topic</Label>
                <Select
                  value={mainPowerForm.topicId}
                  onValueChange={updateMainPowerTopic}
                  required
                ...
                {selectedItemForDetail?.pduList?.length === 0 ? (
                  ...
                  selectedItemForDetail?.pduList.map((pdu, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell>
                        {calculatePUEForPdu(
                          liveSensorValues[selectedItemForDetail.id]?.mainPower !== undefined ? liveSensorValues[selectedItemForDetail.id]?.mainPower : selectedItemForDetail.mainPower?.value,
                          liveSensorValues[selectedItemForDetail.id]?.pduValues[pdu.topicId] !== undefined ? liveSensorValues[selectedItemForDetail.id]?.pduValues[pdu.topicId] : pdu.value
                        )}
                      </TableCell>
                    </TableRow>
                  ))
