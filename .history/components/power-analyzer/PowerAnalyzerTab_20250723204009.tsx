...
import axios from "axios"; // Gunakan axios untuk API calls

// --- UI Components dari Shadcn UI (sesuai yang Anda pakai di project) ---
import {
  Card,
  CardContent,
  CardHeader,
...
// --- Konfigurasi Toast (seperti di BillTab) ---
...
interface MultiselectProps {
  value: string[];
  options: string[];
  placeholder?: string; // Buat placeholder optional
  onChange?: (selected: string[]) => void;
  disabled?: boolean; // Tambahkan disabled
}
...
interface DeviceForSelection {
  uniqId: string; // Ini `id` di Vue, tapi kita pakai `uniqId` dari backend
  name: string;
  topic: string;
...
interface SensorConfig {
  topicUniqId: string; // Ini `topicId` di Vue
  name?: string; // Nama sensor, bisa di-generate atau dari device name
  key: string; // Kunci data yang diambil dari payload
  value: number | null; // Nilai live dari MQTT
...
interface PowerAnalyzerConfig {
  id: string;
  customName: string;
  type: "powerAnalyzer"; // Sesuai default di schema
  ...
  // properti tambahan dari backend yang mungkin di-include
  apiTopic?: DeviceForSelection;
}
...
export function PowerAnalyzerTab() {
  ...
  // --- State Utama ---
  const [powerAnalyzerConfigs, setPowerAnalyzerConfigs] = useState<PowerAnalyzerConfig[]>([]);
  ...
  const [liveSensorValues, setLiveSensorValues] = useState<Record<string, Record<string, number | null>>>({}); // key: configId, value: { sensorUniqId: value }
  ...
  const [listSensorsForm, setListSensorsForm] = useState<SensorConfig[]>([
    { topicUniqId: "", key: "", value: null, topic: undefined, filteredKeys: [] },
  ]);
  const [chartConfigForm, setChartConfigForm] = useState<Record<string, any>>({}); // Untuk chart config, bisa lebih detail
  ...
  const [configToDelete, setConfigToDelete] = useState<PowerAnalyzerConfig | null>(null);
  const [isDeleteAllLogsAlertOpen, setIsDeleteAllLogsAlertOpen] = useState(false);
  ...
  // Ref untuk melacak langganan MQTT aktif untuk configs (mirip PueTab)
  ...
  // Ini fungsi `calculatePUE` dari Vue, kita adaptasi ke React/TS
  const calculatePowerAnalyzerValue = useCallback((config: PowerAnalyzerConfig): number | 'N/A' => {
    ...
    return parseFloat(totalValue.toFixed(2)); // Contoh, sesuaikan perhitungan yang sebenarnya
  }, [liveSensorValues]);
  ...
  const fetchDevicesForSelection = useCallback(async () => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch devices for selection.");
      const data: DeviceForSelection[] = await response.json();
      setDevicesForSelection(data.map(device => ({
        ...device,
        payload: device.lastPayload || {}, // Gunakan lastPayload dari backend
      })));
    } catch (error: any) {
      console.error("[PowerAnalyzerTab] Error fetching devices for selection:", error);
      Toast.fire({ icon: "error", title: `Failed to fetch devices: ${error.message}` });
    }
  }, [getAuthHeaders]);
  ...
  const fetchPowerAnalyzerData = useCallback(async () => {
    ...
    try {
      const [configsRes, logsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/power-analyzer-configs`, { headers: getAuthHeaders() }),
        fetch(`${API_BASE_URL}/api/power-analyzer-logs`, { headers: getAuthHeaders() }),
      ]);

      if (!configsRes.ok) throw new Error("Failed to fetch power analyzer configurations.");
      if (!logsRes.ok) throw new Error("Failed to fetch power analyzer logs.");

      const configsData: PowerAnalyzerConfig[] = await configsRes.json();
      const logsData: PowerAnalyzerLog[] = await logsRes.json();
      
      const initialLiveSensorData: Record<string, Record<string, number | null>> = {};
      
      // Inisialisasi konfigurasi dengan nilai dari lastPayload
      const processedConfigs = configsData.filter(item => item.type === "powerAnalyzer").map(config => {
        const sensorsWithInitialValues = (config.listSensors || []).map(sensor => {
          const device = devicesForSelection.find(d => d.uniqId === sensor.topicUniqId);
          ...
          if (!initialLiveSensorData[config.id]) initialLiveSensorData[config.id] = {};
          initialLiveSensorData[config.id][sensor.topicUniqId] = initialValue; // Simpan nilai awal sensor
      ...
      // Subscribe ke topik-topik relevan
      const topicsToSubscribe = new Set<string>();
      processedConfigs.forEach(config => {
        if (config.apiTopic?.topic) topicsToSubscribe.add(config.apiTopic.topic); // API topic publikasi
        config.listSensors.forEach(sensor => {
          const device = devicesForSelection.find(d => d.uniqId === sensor.topicUniqId);
          if (device?.topic) topicsToSubscribe.add(device.topic); // Sensor device topic
        });
      });

      const currentSubscribedTopics = new Set(configTopicSubscriptionsRef.current.values());
      ...
      currentSubscribedTopics.forEach(topic => {
          if (!topicsToSubscribe.has(topic)) {
              ...
              // Hapus dari ref
              for (let [uniqId, refTopic] of configTopicSubscriptionsRef.current.entries()) {
                  if (refTopic === topic) configTopicSubscriptionsRef.current.delete(uniqId);
              }
          }
      });
      ...
      topicsToSubscribe.forEach(topic => {
          if (!currentSubscribedTopics.has(topic)) {
              ...
              // Tambahkan ke ref (hanya perlu uniqId, ini lebih kompleks karena 1 topic bisa dari banyak uniqId)
              // Logic di handleMqttMessage akan mengupdate berdasarkan topicName
              const device = devicesForSelection.find(d => d.topic === topic);
              if(device) configTopicSubscriptionsRef.current.set(device.uniqId, topic);
          }
      });
      ...
      Toast.fire({ icon: "error", title: `Failed to fetch data: ${error.message}` });
    } finally {
      setIsLoadingInitialData(false);
    }
  }, [getAuthHeaders, subscribe, unsubscribe, devicesForSelection]);


  // --- MQTT Message Handler ---
  const handleMqttMessage = useCallback((topicName: string, messageString: string) => {
    let outerParsedPayload: Record<string, any>;
    try {
      outerParsedPayload = JSON.parse(messageString);
    } catch (e) {
      console.error(`[PowerAnalyzerTab MQTT] Error parsing outer payload for ${topicName}:`, e);
      return;
    }

    let innerValuePayload: Record<string, any> = {};
    if (outerParsedPayload.value && typeof outerParsedPayload.value === "string") {
      try {
        innerValuePayload = JSON.parse(outerParsedPayload.value);
      } catch (e) {
        console.warn(`[PowerAnalyzerTab MQTT] 'value' field not valid JSON string for ${topicName}. Falling back.`);
        innerValuePayload = outerParsedPayload;
      }
    } else if (outerParsedPayload.value && typeof outerParsedPayload.value === "object") {
    ...
    setDevicesForSelection(prevDevices => prevDevices.map(d => {
            ...
            config.listSensors.forEach(sensor => {
                ...
                if (device?.topic === topicName) {
                    if (sensor.key in innerValuePayload) {
                        ...
                        if (!isNaN(val) && currentConfigSensorValues[sensor.topicUniqId] !== val) {
                            currentConfigSensorValues[sensor.topicUniqId] = val;
                            configLiveValuesUpdated = true;
                        }
                    } else {
                        console.warn(`[PowerAnalyzerTab MQTT] Sensor key "${sensor.key}" not found in payload for device ${device.name} on topic ${topicName}.`);
                    }
                }
            });

            // Periksa apakah pesan ini dari apiTopic publikasi PowerAnalyzer itu sendiri
            // Jika ya, itu adalah hasil publikasi, mungkin tidak perlu diupdate di sini
            // Tapi jika Anda ingin PowerAnalyzerTab menampilkan data yang dipublikasikan oleh service,
            // maka Anda harus punya logic untuk parse payload itu.
            // Misalnya:
            // if (config.apiTopic?.topic === topicName && innerValuePayload.pue !== undefined) {
            //    config.pue = parseFloat(innerValuePayload.pue); // Jika ada PUE di payload
            //    configLiveValuesUpdated = true;
            // }
  ...
  }, [powerAnalyzerConfigs, devicesForSelection]); // Tambahkan devicesForSelection sebagai dependensi


  // --- Form Related Functions ---
  const resetForm = useCallback(() => {
    setCustomName("");
    setListSensorsForm([{ topicUniqId: "", key: "", value: null, topic: undefined, filteredKeys: [] }]);
    setChartConfigForm({});
  ...
  const addSensorToForm = () => {
    setListSensorsForm(prev => [...prev, { topicUniqId: "", key: "", value: null, topic: undefined, filteredKeys: [] }]);
  };
  ...
  const removeSensorFromForm = (index: number) => {
    setListSensorsForm(prev => {
      ...
      if (sensorToRemove.topicUniqId) {
        const topicName = devicesForSelection.find(d => d.uniqId === sensorToRemove.topicUniqId)?.topic;
  ...
  const updateSensorTopic = useCallback(async (index: number, selectedUniqId: string) => {
    setListSensorsForm(prev => {
      const newList = [...prev];
      const selectedDeviceObj = devicesForSelection.find(d => d.uniqId === selectedUniqId);
      
      // Unsubscribe previous topic
      const prevSensor = newList[index];
      const prevTopicName = devicesForSelection.find(d => d.uniqId === prevSensor.topicUniqId)?.topic;
      if (prevTopicName && formTopicSubscriptionsRef.current.has(prevTopicName)) {
        unsubscribe(prevTopicName, handleMqttMessage);
        formTopicSubscriptionsRef.current.delete(prevTopicName);
      }

      // Subscribe new topic
      if (selectedDeviceObj?.topic && !formTopicSubscriptionsRef.current.has(selectedDeviceObj.topic)) {
        subscribe(selectedDeviceObj.topic, handleMqttMessage);
        formTopicSubscriptionsRef.current.set(selectedDeviceObj.uniqId, selectedDeviceObj.topic);
      }
      ...
      newList[index] = {
        ...
        filteredKeys: Object.keys(selectedDeviceObj?.payload || {}), // From lastPayload/MQTT
        key: "", // Reset key
      };
      return newList;
    });
  }, [devicesForSelection, subscribe, unsubscribe, handleMqttMessage]);
  ...
  const handleAddData = async (e: FormEvent) => {
    ...
    const isDuplicate = powerAnalyzerConfigs.some(c => c.customName.toLowerCase() === customName.toLowerCase());
    if (isDuplicate) {
    ...
    try {
      // Buat DeviceExternal virtual untuk apiTopic
      const sanitizedCustomName = customName.replace(/\s+/g, "_");
      const apiTopicResponse = await axios.post(
      ...
      const apiTopicUniqId = apiTopicResponse.data.uniqId;
      ...
      const response = await fetch(`${API_BASE_URL}/api/power-analyzer-configs`, {
        method: "POST",
        headers: getAuthHeaders(),
      ...
      await fetchPowerAnalyzerData(); // Refresh all data
    } catch (error: any) {
      console.error("[PowerAnalyzerTab] Error adding config:", error);
      Toast.fire({ icon: "error", title: `Failed to add config: ${error.message}` });
    } finally {
  ...
  const editItem = useCallback((configId: string) => {
    const configToEdit = powerAnalyzerConfigs.find(c => c.id === configId);
    if (!configToEdit) {
      Toast.fire({ icon: "error", title: "Configuration not found for editing." });
      return;
    }
    ...
    const preparedSensorsForm = configToEdit.listSensors.map(sensor => {
      const device = devicesForSelection.find(d => d.uniqId === sensor.topicUniqId);
      // Subscribe to sensor topic for key filtering if not already
      if (device?.topic && !formTopicSubscriptionsRef.current.has(device.topic)) {
      ...
      return {
        ...sensor,
        topic: device,
        filteredKeys: Object.keys(device?.payload || {}), // Use live payload for keys
      };
    });
    setListSensorsForm(preparedSensorsForm);

    setIsEditModalOpen(true);
  }, [powerAnalyzerConfigs, devicesForSelection, subscribe, handleMqttMessage]);


  const handleEditData = async (e: FormEvent) => {
    e.preventDefault();
    if (!selectedConfigId || !customName || listSensorsForm.some(s => !s.topicUniqId || !s.key)) {
      Toast.fire({ icon: "error", title: "Please fill all required fields." });
      return;
    }
    const isDuplicate = powerAnalyzerConfigs.some(c => c.customName.toLowerCase() === customName.toLowerCase() && c.id !== selectedConfigId);
    if (isDuplicate) {
      Toast.fire({ icon: "error", title: "Custom name already exists." });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/power-analyzer-configs/${selectedConfigId}`, {
        method: "PUT",
        headers: getAuthHeaders(),
        body: JSON.stringify(payload),
      });
      ...
      Toast.fire({ icon: "error", title: `Failed to update config: ${error.message}` });
    } finally {
      setIsSubmitting(false);
    }
  };

  const deleteItem = useCallback((configId: string) => {
    setConfigToDelete(powerAnalyzerConfigs.find(c => c.id === configId) || null);
  ...
  const confirmDelete = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/power-analyzer-configs/${configToDelete.id}`, {
        method: "DELETE",
        headers: getAuthHeaders(),
      });
      if (!response.ok) throw new Error((await response.json()).message);
      ...
      Toast.fire({ icon: "error", title: `Failed to delete config: ${error.message}` });
    } finally {
  ...
  const handleDeleteAllLogs = async () => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/power-analyzer-logs/delete-all`, {
            method: 'DELETE',
            headers: getAuthHeaders(),
        });
        if (!response.ok) {
            throw new Error((await response.json()).message || "Failed to delete all logs.");
        }
        ...
        Toast.fire({ icon: "error", title: `Failed to delete all logs: ${error.message}` });
    } finally {
  ...
  useEffect(() => {
    if (devicesForSelection.length > 0 || !isLoadingInitialData) {
  ...
  }, [fetchPowerAnalyzerData, unsubscribe, handleMqttMessage, devicesForSelection, isLoadingInitialData]);
    ...
    <Card className="shadow-sm">
      <CardHeader>
        <div className="flex justify-between items-center">
          ...
          <Button onClick={openAddDataModal} disabled={isLoadingInitialData || isSubmitting || isDeletingConfig || isDeletingAllLogs}>
      ...
      <CardContent>
                  ...
                  <TableRow key={config.id}>
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        disabled={isSubmitting || isLoadingInitialData || isDeletingConfig || isDeletingAllLogs}
                      >
                      ...
                      <Button
                        ...
                        disabled={isSubmitting || isLoadingInitialData || isDeletingConfig || isDeletingAllLogs}
                      >
                        {isDeletingConfig && configToDelete?.id === config.id ? (
        ...
        <div className="flex justify-center mt-4">
          <Button
            ...
            disabled={currentPage === 1 || isLoadingInitialData || isSubmitting || isDeletingConfig || isDeletingAllLogs}
          >
            Previous
          </Button>
          ...
          <Button
            ...
            disabled={currentPage === totalPages || isLoadingInitialData || isSubmitting || isDeletingConfig || isDeletingAllLogs}
          >
            Next
          </Button>
        </div>
      ...
      <Card className="shadow-sm mt-6">
        <CardHeader className="flex flex-row justify-between items-center">
            ...
            <Button variant="destructive" size="sm" onClick={() => setIsDeleteAllLogsAlertOpen(true)} disabled={allLogs.length === 0 || isLoadingInitialData || isSubmitting || isDeletingConfig || isDeletingAllLogs}>
        ...
        <CardContent className="p-0">
            <div className="overflow-x-auto">
                <Table>
                    ...
                    <TableBody>
                        {isLoadingInitialData ? (
                            <TableRow><TableCell colSpan={3} className="text-center h-48"><Loader2 className="mx-auto h-8 w-8 animate-spin text-gray-400" /></TableCell></TableRow>
                        ) : paginatedLogs.length === 0 ? (
                            <TableRow><TableCell colSpan={3} className="text-center h-24 text-muted-foreground">No logs available.</TableCell></TableRow>
                        ) : (
        ...
        <CardFooter className="flex items-center justify-between p-4 border-t">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              disabled={logsPage === 1 || isLoadingInitialData || isSubmitting || isDeletingConfig || isDeletingAllLogs}
            >
              Previous
            </Button>
            <Button
              ...
              disabled={logsPage >= totalLogsPages || isLoadingInitialData || isSubmitting || isDeletingConfig || isDeletingAllLogs}
            >
              Next
            </Button>
          </div>
              ...
              {listSensorsForm.map((sensor, index) => (
                <div key={index} className="border p-4 rounded-md mb-2">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={(value) => updateSensorTopic(index, value)}
                        required
                        disabled={isSubmitting}
                      >
                        <SelectTrigger>
                    ...
                    <div>
                      ...
                      <Select
                        ...
                        required
                        disabled={isSubmitting || (sensor.filteredKeys?.length || 0) === 0}
                      >
                        <SelectTrigger>
                          <SelectValue placeholder={sensor.filteredKeys?.length ? "Select a key" : "No keys (select device first)"} />
                        </SelectTrigger>
                        <SelectContent>
                  ...
                  {listSensorsForm.length > 1 && (
                    <Button type="button" variant="destructive" size="sm" className="mt-2" onClick={() => removeSensorFromForm(index)} disabled={isSubmitting}>
                      Remove Sensor
                    </Button>
                  )}
                </div>
              ))}
              <Button type="button" variant="outline" onClick={addSensorToForm} disabled={isSubmitting}>
      ...
      <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
        <DialogContent className="sm:max-w-[800px]">
          ...
          <form onSubmit={handleEditData}>
            ...
            <DialogFooter>
              <Button type="button" variant="ghost" onClick={() => setIsEditModalOpen(false)} disabled={isSubmitting}>
                Cancel
              </Button>
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
      ...
      <AlertDialog open={isDeleteAlertOpen} onOpenChange={setIsDeleteAlertOpen}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeletingConfig} onClick={() => setConfigToDelete(null)}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction onClick={confirmDelete} disabled={isDeletingConfig} className="bg-red-600 hover:bg-red-700">
              {isDeletingConfig ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : null}
              Delete
      ...
      <AlertDialog open={isDeleteAllLogsAlertOpen} onOpenChange={setIsDeleteAllLogsAlertOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Are you absolutely sure?</DialogTitle>
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete ALL Power Analyzer logs from the database.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={isDeletingAllLogs}>Cancel</AlertDialogCancel>
            <AlertDialogAction onClick={handleDeleteAllLogs} disabled={isDeletingAllLogs} className="bg-red-600 hover:bg-red-700">
              {isDeletingAllLogs ? (
                <Loader2 className="mr-2 h-4 w-4 animate-spin" />
              ) : null}
              Yes, delete all
