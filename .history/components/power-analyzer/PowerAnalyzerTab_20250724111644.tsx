...
import { useMqtt } from "@/contexts/MqttContext"; // Sesuaikan path jika perlu
...
export function PowerAnalyzerTab() {
    ...
    try {
      ...
      if (typeof parsedPayload.value === "string") {
        try {
          ...
          /* Abaikan jika gagal parse, gunakan payload asli */
        }
      }
  ...
  // Efek untuk cleanup subscribe saat modal ditutup
  useEffect(() => {
    return () => {
  ...
  // Efek untuk subscribe data di tabel utama
  useEffect(() => {
  ...
  const confirmDelete = async () => {
    ...
    toast
      .promise(axios.delete(`/api/power-analyzer/${deletingId}`), {
