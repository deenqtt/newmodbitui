...
export function PueTab() {
    ...
    try {
      ...
      const fetchedPueConfigs: PueConfig[] = data
        ...
        .map((item) => {
          ...
          if (mainPowerDevice && mainPowerDevice.payload && item.mainPower.key in mainPowerDevice.payload) {
              const parsedValue = parseFloat(mainPowerDevice.payload[item.mainPower.key]);
              if (!isNaN(parsedValue)) {
                  currentMainPowerValue = parsedValue;
              } else {
                  console.warn(`[PUE Tab] Main Power Key "${item.mainPower.key}" has non-numeric value in initial payload: ${mainPowerDevice.payload[item.mainPower.key]}`);
              }
          }
                ...
                if (key in pduDevice.payload) {
                  ...
                  if (!isNaN(val)) {
                    ...
                    hasValidValue = true;
                      ...
                      console.warn(`[PUE Tab] PDU Key "${key}" has non-numeric value in initial payload: ${pduDevice.payload[key]} for PDU ${pdu.name}`);
                  }
                } else {
                    console.warn(`[PUE Tab] PDU Key "${key}" not found in initial payload for PDU ${pdu.name}. Payload keys: ${Object.keys(pduDevice.payload)}`);
                }
              });
  ...
  }, [getAuthHeaders, calculatePUE, subscribeToRelevantTopics, devicesForSelection]); // <-- PENTING: Tambahkan `devicesForSelection` sebagai dependensi!
  ...
  const editItem = (id: string) => {
    ...
    if (itemToEdit) {
      ...
      setMainPowerForm({
        ...
        value: itemToEdit.mainPower.value, // Menggunakan nilai dari itemToEdit (yang mungkin dari lastPayload)
      ...
      const newPduListForm = itemToEdit.pduList.map((pdu) => {
        ...
        return {
          ...
          name: pdu.name,
          keys: pdu.keys,
          value: pdu.value, // Menggunakan nilai dari itemToEdit (yang mungkin dari lastPayload)
          topic: pduDeviceObj,
  ...
  useEffect(() => {
    ...
    if (devicesForSelection.length > 0) { // Hanya panggil fetchData jika devicesForSelection sudah ada isinya
        fetchData();
    }
