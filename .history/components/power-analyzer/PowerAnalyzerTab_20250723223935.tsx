...
export function PowerAnalyzerTab() {
  ...
  const fetchDevicesForSelection = useCallback(async () => {
    try {
      ...
      if (!response.ok)
        throw new Error("Failed to fetch devices for selection.");
  ...
  const fetchPowerAnalyzerData = useCallback(async () => {
    ...
    try {
      const [configsRes, logsRes] = await Promise.all([
        fetch(`${API_BASE_URL}/api/power-analyzer-configs`, {
          headers: getAuthHeaders(),
        }),
        fetch(`${API_BASE_URL}/api/power-analyzer-logs`, {
          headers: getAuthHeaders(),
        }),
      ]);

      if (!configsRes.ok)
        throw new Error("Failed to fetch power analyzer configurations.");
      ...
      processedConfigs.forEach((config) => {
        if (config.apiTopic?.topic)
          topicsToSubscribe.add(config.apiTopic.topic); // API topic publikasi
        config.listSensors.forEach((sensor) => {
          const device = devicesForSelection.find(
            (d) => d.uniqId === sensor.topicUniqId
          );
  ...
  const handleMqttMessage = useCallback(
    ...
    [powerAnalyzerConfigs, devicesForSelection]
  ); // Tambahkan devicesForSelection sebagai dependensi
  ...
  const handleDeleteAllLogs = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          (await response.json()).message || "Failed to delete all logs."
        );
      }
  ...
  useEffect(() => {
    // Kondisi untuk memicu fetchPowerAnalyzerData:
    // 1. `devicesForSelection` sudah terisi (setidaknya ada data device)
    // Ini untuk memastikan fetch terjadi saat ada data devices.
    ...
    handleMqttMessage,
    devicesForSelection,
  ]); // `isLoadingInitialData` dihapus dari dependensi
