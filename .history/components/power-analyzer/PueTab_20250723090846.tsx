...
export function PueTab() {
  ...
  const handleMqttMessage = useCallback(
    (topicName: string, messageString: string) => {
      ...
      try {
        ...
        console.error(`[PUE Tab] Error parsing outer MQTT payload for topic ${topicName}:`, e);
        return;
      }
      ...
      if (outerParsedPayload.value && typeof outerParsedPayload.value === 'string') {
        try {
          ...
          console.log("[PUE Tab] Inner 'value' Payload (parsed):", innerValuePayload);
        } catch (e) {
          console.warn(`[PUE Tab] Warning: 'value' field in payload for topic ${topicName} is not valid JSON string. Falling back to outer payload.`, outerParsedPayload.value, e);
        ...
        console.log("[PUE Tab] Inner 'value' Payload (already object):", innerValuePayload);
      } else {
        // Jika tidak ada field 'value' atau bukan string/objek, gunakan seluruh payload terluar
        innerValuePayload = outerParsedPayload;
        console.log("[PUE Tab] No 'value' field or not string/object. Using full outer payload for keys:", innerValuePayload);
      }
      ...
      setDevicesForSelection((prevDevices) =>
        prevDevices.map((d) => {
          if (d.topic === topicName) {
            console.log(`[PUE Tab] Updating payload for device: ${d.name} (${d.uniqId}) with new data.`);
      ...
      setPueConfigs((prevConfigs) => {
        ...
        const newConfigs = prevConfigs.map((config) => {
          ...
          const newPduList = config.pduList.map((pdu) => {
            ...
            if (pduTopicName === topicName) {
              ...
              pdu.keys.forEach((key) => {
                if (key in innerValuePayload) { // Gunakan innerValuePayload di sini
                  const value = parseFloat(innerValuePayload[key]);
                  if (!isNaN(value)) {
                    totalValue += value;
                  } else {
                    console.warn(`[PUE Tab] Key "${key}" has non-numeric value: ${innerValuePayload[key]} for PDU config ${config.customName}`);
                  }
                } else {
                  console.warn(`[PUE Tab] Key "${key}" not found in payload for PDU config ${config.customName}. Payload keys: ${Object.keys(innerValuePayload)}`);
                }
              });
          ...
          if (mainPowerTopicName === topicName) {
            ...
            if (mainKey in innerValuePayload) { // Gunakan innerValuePayload di sini
              ...
              if (!isNaN(value)) {
                ...
                console.warn(`[PUE Tab] Main Power Key "${mainKey}" has non-numeric value: ${innerValuePayload[mainKey]} for config ${config.customName}`);
              }
            } else {
              console.warn(`[PUE Tab] Main Power Key "${mainKey}" not found in payload for config ${config.customName}. Payload keys: ${Object.keys(innerValuePayload)}`);
            }
          }
          ...
          if (currentConfigUpdated) {
            ...
            console.log(`[PUE Tab] Config '${config.customName}' updated by MQTT. New PUE: ${updatedConfig.pue}`);
        ...
        if (configsUpdated) {
           console.log("[PUE Tab] At least one PUE config was updated by MQTT message.");
        }
        return newConfigs;
      });
    },
  ...
  const subscribeToRelevantTopics = useCallback(async (configs: PueConfig[]) => {
    ...
    console.log("[PUE Tab] All unique Topic UniqIds to potentially subscribe:", Array.from(allTopicUniqIdsToSubscribe));

    for (const topicUniqId of Array.from(allTopicUniqIdsToSubscribe)) {
      try {
        console.log(`[PUE Tab] Fetching topic name for UniqId: ${topicUniqId}`);
        const response = await fetch(`${API_BASE_URL}/api/topics/by-uniqid/${topicUniqId}`, {
          headers: getAuthHeaders(),
        });
        if (!response.ok) {
          throw new Error(`HTTP error! status: ${response.status}`);
        ...
        console.log(`[PUE Tab] Mapped UniqId ${topicUniqId} to Topic Name: ${data.topicName}`);
      } catch (error) {
        console.warn(`[PUE Tab] Failed to get topic name for UniqId ${topicUniqId}:`, error);
      }
    }
    console.log("[PUE Tab] Current Topic UniqId to Name Map:", currentTopicUniqIdToNameMap);
    console.log("[PUE Tab] Previous Topic UniqId to Name Map:", topicUniqIdToTopicNameMapRef.current);


    topicUniqIdToTopicNameMapRef.current.forEach((topicName, topicUniqId) => {
      if (!currentTopicUniqIdToNameMap.has(topicUniqId)) {
        console.log(`[PUE Tab] Unsubscribing from old MQTT topic: ${topicName} (UniqId: ${topicUniqId})`);
    ...
    currentTopicUniqIdToNameMap.forEach((topicName, topicUniqId) => {
      if (!topicUniqIdToTopicNameMapRef.current.has(topicUniqId)) {
        console.log(`[PUE Tab] Subscribing to new/relevant MQTT topic: ${topicName} (UniqId: ${topicUniqId})`);
        subscribe(topicName, handleMqttMessage);
      } else {
        console.log(`[PUE Tab] Already subscribed to MQTT topic: ${topicName} (UniqId: ${topicUniqId})`);
      }
    });
  ...
  const updatePduTopic = (index: number, selectedTopicUniqId: string) => {
    console.log(`[PUE Tab] Updating PDU topic for index ${index}. Selected UniqId: ${selectedTopicUniqId}`);
    setPduListForm((prev) => {
      ...
      console.log(`[PUE Tab] Found selected device object for PDU:`, selectedDeviceObj);
      console.log(`[PUE Tab] Payload used for PDU keys:`, selectedDeviceObj?.payload);
      ...
      newPduList[index].filteredKeys = keysFromPayload.filter(
        (key) => key.toLowerCase().includes("power") || key.toLowerCase().includes("lux")
      );
      newPduList[index].keys = []; // Reset selected keys
      console.log(`[PUE Tab] PDU filtered keys for index ${index}:`, newPduList[index].filteredKeys);
      return newPduList;
    });
  };

  const updatePduKeys = (index: number, selectedKeys: string[]) => {
    console.log(`[PUE Tab] Updating PDU selected keys for index ${index}:`, selectedKeys);
    setPduListForm((prev) => {
      const newPduList = [...prev];
  ...
  const updateMainPowerTopic = (selectedTopicUniqId: string) => {
    console.log(`[PUE Tab] Updating Main Power topic. Selected UniqId: ${selectedTopicUniqId}`);
    const selectedDeviceObj = devicesForSelection.find((d) => d.uniqId === selectedTopicUniqId);
    setMainPowerForm((prev) => ({
      ...prev,
      topicUniqId: selectedTopicUniqId,
      topic: selectedDeviceObj,
      console.log(`[PUE Tab] Found selected device object for Main Power:`, selectedDeviceObj),
      console.log(`[PUE Tab] Payload used for Main Power keys:`, selectedDeviceObj?.payload),
      // Filter keys dari payload yang sudah di-parse (innerValuePayload)
    ...
    console.log(`[PUE Tab] Main Power filtered keys:`, mainPowerForm.filteredKeys);
  };
  ...
  const handleEditData = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`[PUE Tab] PUE configuration with ID ${selectedPueConfigId} updated successfully.`);
  ...
  const deleteItem = async (id: string) => {
    ...
    if (confirmResult.isConfirmed) {
      try {
        ...
        console.log(`[PUE Tab] PUE configuration with ID ${id} deleted successfully.`);
  ...
  useEffect(() => {
    ...
    return () => {
      console.log("[PUE Tab] Unsubscribing from all MQTT topics on PueTab unmount cleanup.");
      ...
      <CardContent>
          ...
          <DialogContent className="sm:max-w-[800px]">
            ...
            <form onSubmit={handleEditData}>
              <div className="grid gap-4 py-4">
                ...
                <h6 className="font-semibold mt-3">Main Power</h6>
                <div className="space-y-2">
                  <label htmlFor="mainPowerTopicSelectionEdit" className="text-sm font-medium">Select Device</label>
                  <Select
                    value={mainPowerForm.topicUniqId}
                    onValueChange={updateMainPowerTopic}
                  >
                ...
                <div className="space-y-2">
                  <label htmlFor="mainPowerKeySelectionEdit" className="text-sm font-medium">Select Key</label>
                  <Select
                    value={mainPowerForm.key}
                    onValueChange={updateMainPowerKey}
                  >
        ...
        <Dialog open={isDetailModalOpen} onOpenChange={setIsDetailModalOpen}>
          <DialogContent className="sm:max-w-[600px]">
            <DialogHeader>
              <DialogTitle>Detail PUE for {selectedPueConfigDetail?.customName}</DialogTitle>
            </DialogHeader>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">PUE</th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {selectedPueConfigDetail?.pduList?.length === 0 ? (
                    ...
                    selectedPueConfigDetail?.pduList?.map((pdu, index) => (
                      <tr key={index}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">{pdu.name || `PDU-${index + 1}`}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                          {calculatePUEForPdu(selectedPueConfigDetail.mainPower.value, pdu.value)}
                        </td>
                      </tr>
                    ))
                  )}
