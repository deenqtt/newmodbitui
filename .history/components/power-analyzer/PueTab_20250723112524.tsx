...
export function PueTab() {
    ...
    try {
      ...
      setDevicesForSelection(
        data.map((device: DeviceForSelection) => ({
          ...device,
          payload: device.lastPayload || {}, // <-- NEW: Gunakan `lastPayload` dari API
        })) // <-- Pastikan kurung ini ada dan pasangannya benar
      ); // <-- Pastikan kurung ini ada dan pasangannya benar
    } catch (error) {
      // <-- Ini baris 227 di error Anda
  ...
  const handleMqttMessage = useCallback(
    (topicName: string, messageString: string) => {
      ...
      // Juga update `lastPayload` dan `lastUpdatedByMqtt` jika Anda berencana menyimpan di frontend cache
        ...
        prevDevices.map((d) => {
          if (d.topic === topicName) {
            ...
            return {
              ...
              // Jika Anda ingin menyimpan lastPayload di frontend (opsional)
      ...
      setPduListForm((prev) => {
        ...
        const newPduList = prev.map((pdu) => {
          // Cari device yang di-update di devicesForSelection state
    ...
    [calculatePUE, devicesForSelection] // `devicesForSelection` as dependency is crucial
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const fetchedPueConfigs: PueConfig[] = data
        ...
        .map((item) => {
          // Dapatkan device yang sesuai dari `devicesForSelection`
          // Penting: devicesForSelection harus sudah diisi dengan lastPayload
          const mainPowerDevice = devicesForSelection.find(
            (d) => d.uniqId === item.mainPower.topicUniqId
          );

          // Dapatkan nilai Main Power dari payload terakhir jika ada
      ...
      fetchedPueConfigs.forEach((config) => {
        // PUE akan dihitung dengan nilai yang sudah diambil dari lastPayload (jika ada) atau null
    ...
    devicesForSelection,
  ]); // <-- PENTING: Tambahkan `devicesForSelection` sebagai dependensi!

  const resetForm = useCallback(() => {
    // Unsubscribe from all topics tracked by the form when resetting
  ...
  const handleRemovePdu = (index: number) => {
    // Unsubscribe from the specific PDU's topic if it was subscribed for the form
    setPduListForm((prev) => {
  ...
  const handleAddData = async (e: React.FormEvent) => {
    ...
    try {
      ...
      resetForm();
      await fetchData(); // Refetch all configs and manage subscriptions for them
      await fetchDevicesForSelection(); // Ensure devices list is fresh with potential new lastPayloads
    } catch (error: any) {
  ...
  const editItem = (id: string) => {
    ...
    if (itemToEdit) {
      ...
      // --- NEW/UPDATED: Logic for Main Power Form ---
      const mainPowerDeviceObj = devicesForSelection.find(
        (d) => d.uniqId === itemToEdit.mainPower.topicUniqId
      );

      // Pastikan untuk subscribe ke topik Main Power jika belum di-subscribe oleh main config
      // dan tambahkan ke formTopicSubscriptionRef
      if (
      ...
      // Ambil filteredKeys dari payload `mainPowerDeviceObj` (yang kini harusnya punya lastPayload)
      ...
      setMainPowerForm({
        ...
        value: itemToEdit.mainPower.value, // Ini akan menggunakan nilai dari itemToEdit
        topic: mainPowerDeviceObj,
        filteredKeys: mainPowerFilteredKeys, // Gunakan filteredKeys yang sudah diambil
      });
      // Log ini mungkin menunjukkan state sebelumnya, tidak masalah.
      ...
      // --- NEW/UPDATED: Logic for PDU List Form ---
      const newPduListForm = itemToEdit.pduList.map((pdu) => {
        const pduDeviceObj = devicesForSelection.find(
          (d) => d.uniqId === pdu.topicUniqId
        );

        // Pastikan untuk subscribe ke topik PDU jika belum di-subscribe oleh main config
        // dan tambahkan ke formTopicSubscriptionRef
        if (
          pduDeviceObj?.topic &&
        ...
        // Ambil filteredKeys dari payload `pduDeviceObj`
        ...
        return {
          ...
          name: pdu.name,
          keys: pdu.keys,
          value: pdu.value, // Ini akan menggunakan nilai dari itemToEdit
          topic: pduDeviceObj,
          filteredKeys: pduFilteredKeys, // Gunakan filteredKeys yang sudah diambil
        };
      });
      ...
      setIsEditModalOpen(true); // <-- Pastikan ini dipanggil di akhir
    } else {
    ...
    try {
      const payload: Partial<PueConfig> = {
        ...
        pduList: pduListForm.map((pdu, index) => ({
          ...
          name: pdu.name || `PDU-${index + 1}`,
          keys: pdu.keys,
          value: null, // Tetap null saat disimpan ke DB
        })),
        mainPower: {
          topicUniqId: mainPowerForm.topicUniqId,
          key: mainPowerForm.key,
  ...
  useEffect(() => {
    // Memastikan `fetchDevicesForSelection` dijalankan lebih dulu
    // agar `devicesForSelection` memiliki `lastPayload` sebelum `fetchData` memprosesnya.
    fetchDevicesForSelection();
  }, [fetchDevicesForSelection]);

  useEffect(() => {
    // `fetchData` sekarang bergantung pada `devicesForSelection`
    // sehingga akan re-run ketika `devicesForSelection` terisi (termasuk lastPayload)
    // atau ketika ada perubahan pada devicesForSelection
    if (devicesForSelection.length > 0) {
      // Hanya panggil fetchData jika devicesForSelection sudah ada isinya
      fetchData();
    }
  ...
  }, [fetchData, unsubscribe, handleMqttMessage, devicesForSelection]); // Tambahkan devicesForSelection di sini juga
              ...
              {paginatedData.length === 0 ? (
                ...
                paginatedData.map((item, index) => (
                  <tr key={item.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                      {/* NEW: Asumsi item.pue akan langsung terisi dengan nilai atau "N/A" dari calculatePUE */}
                      {/* "Calculating..." hanya akan muncul jika `lastPayload` di database juga null/kosong */}
                      {item.pue}
                      <Button
                        variant="outline"
                        size="sm"
