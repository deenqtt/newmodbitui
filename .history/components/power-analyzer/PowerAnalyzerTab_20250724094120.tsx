...
export function PowerAnalyzerTab() {
  ...
  return (
    ...
    // Ganti seluruh blok <Dialog> yang ada dengan ini
  ...
  <DialogContent className="max-w-2xl">
    ...
    <div className="grid gap-4 py-4 max-h-[60vh] overflow-y-auto pr-2">
        ...
        {pduList.map((pdu, index) => (
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                    ...
                    <Select value={pdu.uniqId ?? undefined} onValueChange={(value) => handlePduChange(index, 'uniqId', value)}>
                ...
                <div className="space-y-2">
                    ...
                    <MultiSelect options={getKeyOptions(pdu.uniqId)} isMulti value={getKeyOptions(pdu.uniqId).filter(o => pdu.keys.includes(o.value))} onChange={(opts: any) => handlePduChange(index, 'keys', opts.map((o: any) => o.value))} isDisabled={!pdu.uniqId} placeholder="Select keys..."/>
                </div>
            </div>
        ...
        <div className="grid grid-cols-2 gap-4 rounded-lg border p-4">
            ...
            <Select value={mainPower.uniqId ?? undefined} onValueChange={(value) => setMainPower({ uniqId: value, key: null })}>
                <SelectTrigger><SelectValue placeholder="Select a topic..." /></SelectTrigger>
                <SelectContent>{deviceOptions.map(opt => <SelectItem key={opt.value} value={opt.value}>{opt.label}</SelectItem>)}</SelectContent>
            </Select>
            </div>
            ...
            selectedConfig.pduList.map((pdu, index) => {
              ...
              return (
                <TableRow key={index}>
                  ...
                  <TableCell>{calculatePUEForPdu(mainPowerValue, pduValue)}</TableCell>
                </TableRow>
              );
            })
          )}
