...
export function PowerAnalyzerTab() {
    ...
    try {
      ...
      if (typeof parsedPayload.value === "string") {
        try {
          ...
          /* Ignore if nested parsing fails */
        }
      }
  ...
  const getKeyOptions = useCallback(
    (uniqId: string | null): SelectOption[] => {
      ...
      // Prioritas 1: Cek apakah ada JSON di dalam properti 'value'
      if (typeof rawPayload.value === "string") {
        try {
          // Jika berhasil di-parse, gunakan HANYA objek ini sebagai sumber data
          dataObject = JSON.parse(rawPayload.value);
        } catch (e) {
          // Jika gagal parse, biarkan dataObject null agar bisa fallback
          console.warn(
            "Could not parse payload.value, will check raw payload."
          );
        }
      }

      // Prioritas 2 (Fallback): Jika tidak ada 'value' atau gagal parse,
      // gunakan seluruh payload sebagai sumber data.
      if (dataObject === null) {
        dataObject = rawPayload;
      }

      // Safety check jika dataObject tetap tidak valid
      if (typeof dataObject !== "object" || dataObject === null) {
        return [];
      }

      // Filter key dari dataObject yang sudah ditentukan
