    ...
    return Object.entries(menuConfig)
      ...
      .map(([menuKey, menuData]) => ({
        ...
        items: Object.entries(menuData.submenus)
          ...
          .map(([submenuTitle]) => {
            // Logika URL disesuaikan dengan struktur folder Anda
            let parentPath = toKebabCase(menuKey);
            let childPath = toKebabCase(submenuTitle);

            let url = `/${parentPath}/${childPath}`;
            if (submenuTitle === "Main Dashboard") {
              url = "/"; // Atau sesuaikan dengan path dashboard utama Anda
            }
