...
class AlarmMonitorService {
  ...
  private async loadAlarmConfigurations(): Promise<string[]> {
    console.log(
      "[ALARM SERVICE] Loading alarm configurations from database..."
    );
    ...
    console.log(
      `[ALARM SERVICE] Loaded ${configsFromDb.length} alarm configs to monitor.`
    );
    ...
    this.mqttClient.on("connect", () => {
      ...
      if (topics.length > 0) {
        this.mqttClient?.subscribe(topics, (err) => {
          if (!err) {
            console.log(
              `[ALARM SERVICE] Subscribed to ${topics.length} topics for alarm monitoring.`
            );
          }
        });
      }
  ...
  private handleMessage(topic: string, payloadStr: string) {
    ...
    try {
      ...
      console.error(
        `[ALARM SERVICE] Error processing message on topic ${topic}:`,
        error
      );
    }
  }
  ...
  private async logAlarmEvent(
    ...
    console.log(
      `[ALARM EVENT] Alarm: "${alarmName}", Status: ${status}, Value: ${value}`
    );
    try {
      if (isActive) {
      ...
      console.error(
        `[ALARM EVENT] Failed to log alarm event for "${alarmName}":`,
        error
      );
    }
  }
