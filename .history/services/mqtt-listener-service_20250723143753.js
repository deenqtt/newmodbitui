...
// Map untuk melacak timer publikasi PUE agar tidak terlalu sering
// key: pueConfig.id, value: { timer: setTimeoutId, lastPublishedValue: any, pendingUpdate: boolean }
  ...
  try {
    ...
    client.on("message", async (topic, message) => {
      ...
      for (const pueConfig of relevantPueConfigs) {
        // Cek apakah ada timer yang sedang berjalan untuk konfigurasi PUE ini
        const timerInfo = puePublicationTimers.get(pueConfig.id);
        const now = Date.now();

        // Jika tidak ada timer atau sudah waktunya untuk publish
        if (
          !timerInfo ||
          now - timerInfo.lastPublishedTime >= PUBLISH_INTERVAL_MS ||
          timerInfo.pendingUpdate
        ) {
          ...
          try {
            ...
            const calculatedPUE = calculatePUE(
              mainPowerValue,
              validPduDataList
            );
            ...
            // Format payload sesuai permintaan Anda
            const puePublishPayload = {
              ...
              value: {
                pue: parseFloat(calculatedPUE), // Pastikan ini angka jika ingin di dalam value
                mainPowerValue_W: mainPowerValue, // Menggunakan satuan Watt
                itPower_W: itPower, // Menggunakan satuan Watt
              },
            };
            ...
            if (pueConfig.apiTopic && pueConfig.apiTopic.topic) {
              ...
              console.log(
                `[MQTT Service] PUBLISHED PUE to '${
                  pueConfig.apiTopic.topic
                }' every ${PUBLISH_INTERVAL_MS / 1000}s. Payload:`,
                puePublishPayload
              );
              // Reset timer dan status
              puePublicationTimers.set(pueConfig.id, {
                timer: null,
          ...
          // Jika ada timer yang sedang berjalan, tandai ada update yang tertunda
          console.log(
            `[MQTT Service] PUE calculation for '${pueConfig.customName}' pending publication. Will publish in next cycle.`
          );
          puePublicationTimers.set(pueConfig.id, {
            ...timerInfo,
            pendingUpdate: true,
    ...
    client.on("close", () => {
      ...
      // Clear all existing timers on disconnect to prevent multiple timers on reconnect
