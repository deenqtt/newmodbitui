...
// Ini penting untuk perhitungan PUE yang membutuhkan nilai dari beberapa device
...
async function startService() {
  ...
  try {
    ...
    client.on("connect", async () => {
      // Gunakan async karena ada await di dalamnya
      ...
      const topicsToSubscribe = new Set(); // Gunakan Set untuk menghindari duplikasi topik
    ...
    client.on("message", async (topic, message) => {
      ...
      if (
        ...
        try {
          ...
          // console.log("[MQTT Service] Inner 'value' Payload (parsed):", innerValuePayload); // Log ini bisa sangat verbose
        } catch (e) {
          console.warn(
        ...
        // console.log("[MQTT Service] Inner 'value' Payload (already object):", innerValuePayload); // Log ini bisa sangat verbose
      } else {
        innerValuePayload = outerParsedPayload;
        // console.log("[MQTT Service] No 'value' field or not string/object. Using full outer payload for keys:", innerValuePayload); // Log ini bisa sangat verbose
      }
      // --- END Logika Parsing Inner Payload ---

      // --- Update lastKnownPayloads map dan DeviceExternal di database ---
      let updatedDeviceUniqId = null;
      let deviceToUpdateInDb;

      // Cari device berdasarkan topik yang diterima
      // Kita perlu iterasi `lastKnownPayloads` karena `Map` tidak bisa di-query berdasarkan value (topic)
      ...
      // Jika tidak ditemukan di `lastKnownPayloads` (misalnya service baru jalan dan belum sempat fetch semua),
      // coba cari di database dan tambahkan ke `lastKnownPayloads`
      if (!updatedDeviceUniqId) {
        deviceToUpdateInDb = await prisma.deviceExternal.findUnique({
          where: { topic: topic },
          select: { uniqId: true, topic: true },
        });
        if (deviceToUpdateInDb) {
          updatedDeviceUniqId = deviceToUpdateInDb.uniqId;
          ...
          return; // Jika tidak ada device yang cocok di DB atau cache, abaikan pesan ini
        }
      }
      ...
      for (const config of relevantBillConfigs) {
        try {
          ...
          console.log(
            `[MQTT Service] Log Bill disimpan untuk '${config.customName}'. Raw Value: ${rawValue}`
          );
          ...
          console.log(
            `[MQTT Service] Hasil Bill dipublikasikan ke topik '${config.publishTargetDevice.topic}'.`
          );
        } catch (error) {
          console.error(
      ...
      const relevantPueConfigs = pueConfigs.filter((config) => {
        ...
        // Kita perlu mencari uniqId dari topic yang diterima, lalu lihat apakah uniqId itu ada di config PUE
      ...
      for (const pueConfig of relevantPueConfigs) {
        try {
          ...
          console.log(
            `[MQTT Service] PUE calculated for '${pueConfig.customName}': ${calculatedPUE}`
          );
          ...
          if (pueConfig.apiTopic && pueConfig.apiTopic.topic) {
            const puePublishPayload = {
              ...
              itPower: validPduDataList.reduce(
