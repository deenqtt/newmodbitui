...
def handle_device_topic_data(client, topic, payload):
    """Handle incoming device data from subscribed topics and remap/publish with group buffering"""
    try:
            ...
            for remap_config in config:
                ...
                for device in remap_config.get('source_devices', []):
                    if device.get('mqtt_topic') == topic:
                        ...
                        # Store the real-time data for this device (but don't publish immediately)
                        if remapped_data:
                            # Only buffer the data without publishing - let periodic publishing handle it
                            device_group = device.get('group')
                            timestamp = device_message.get('Timestamp', datetime.now().isoformat())

                            # Store in cache for potential later use, but don't trigger publish
                            add_to_device_cache(config_id, topic, remapped_data, timestamp)

                            log_simple(f"Device {topic} data buffered (not publishing) - periodic publish will handle", "INFO")
