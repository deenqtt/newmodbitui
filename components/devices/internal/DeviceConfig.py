...
MODBIT_I2C_CONFIG_PATH = '../I2C/JSON/Config/installed_devices.json'
...
MQTT_MODBIT_CONFIG_PATH = '../I2C/JSON/Config/mqtt_config.json'
...
I2C_MODBIT_COMMAND_TOPIC = "command_device_i2c_plain"
I2C_MODBIT_RESPONSE_TOPIC = "response_device_i2c_plain"
I2C_MODBIT_DATA_TOPIC = "data_device_i2c_plain_node"
# Error log topic
...
def handle_i2c_modbit_message(client, userdata, message):
    print(f"Received I2C PLAIN message: {message.payload.decode('utf-8')} on topic {message.topic}")
    payload = message.payload.decode('utf-8')
    print(f"Raw payload: {payload}")
    installed_devices = load_installed_devices(client, MODBIT_I2C_CONFIG_PATH)
    ...
    try:
        ...
        if command.get('command') == 'getDataI2C':
            ...
            for device in installed_devices:
                if device['profile']['name'] == new_device['profile']['name'] or device['protocol_setting']['address'] == new_device['protocol_setting']['address']:
                    response = {"status": "error", "message": "Device with the same name or address already exists."}
                    client.publish(I2C_MODBIT_RESPONSE_TOPIC, json.dumps(response), qos=1, retain=False)
                    return

            installed_devices.append(new_device)
            save_installed_devices(client, MODBIT_I2C_CONFIG_PATH, installed_devices)
            response = {"status": "success", "message": "Device added successfully"}
        ...
        client.publish(I2C_MODBIT_RESPONSE_TOPIC, json.dumps(response), qos=0, retain=False)
...
def send_device_data(client, device_type, data):
    try:
        ...
        elif device_type == "i2c_modbit":
            topic = I2C_MODBIT_DATA_TOPIC
        else:
...
def periodic_publish(client):
    while True:
        try:
            ...
            i2c_devices_modbit = load_installed_devices(client, MODBIT_I2C_CONFIG_PATH)

            send_device_data(client, "modbus", modbus_devices)
            send_device_data(client, "i2c", i2c_devices)
            send_device_data(client, "i2c_modbit", i2c_devices_modbit)  # Publish ke topic baru
...
def on_connect_operations(client, userdata, flags, rc):
    if rc == 0:
        ...
        client.subscribe(I2C_MODBIT_COMMAND_TOPIC)  # subscribe topik baru
    else:
...
def setup_mqtt_client_operations():
    ...
    client.message_callback_add(I2C_MODBIT_COMMAND_TOPIC, handle_i2c_modbit_message)  # handler baru
